...
// A directory carrying its own copies of the shared modules — the shape a package-local
// React install leaves in packages/factorylm-ui outside the workspace. Built here rather than mutating the real tree so the negative
...
describe("findReactDuplication", () => {
  test("is silent when the lab and @factorylm/ui resolve to one instance", () => {
    // The real tree, as the workspace install leaves it. If this ever returns a
    // message, the workspace no longer provides one React and the shell suites
    // are about to fail with "Invalid hook call".
  ...
  test("reports a module that does not resolve from the package at all", () => {
    // The other failure shape — the package's dependencies were never installed
    // (a tree installed outside the workspace), which surfaces as `Cannot find package 'react'`.
