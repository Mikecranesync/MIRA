...
def _print_summary(evaluated: list[EvaluatedResult], config: RunConfig) -> None:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default="dry-run",
        choices=["dry-run", "bot-only", "sidecar-only", "both"],
...
async def _main(args: argparse.Namespace) -> int:
    ...
    json_path = _write_json_report(results, evaluated, config, output_dir, ts_str)
    ...
    # 4. Print summary.
    _print_summary(evaluated, config)
