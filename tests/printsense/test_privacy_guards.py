...
def test_1_no_tracked_images():
    ...
    assert not image_files, f"Found tracked image files under printsense/: {image_files}"
...
def test_2_gitignore_patterns():
    ...
    for pattern in required_patterns:
        assert pattern in content, f"Required pattern '{pattern}' not found in {gitignore_path}"
...
def _scan_tracked(repo_root: Path, hash_set: set[str]) -> list[tuple[str, str]]:
    ...
    tracked_files = [f for f in output.split("\0") if f]
    relevant_files = [
        f for f in tracked_files if f.startswith(("printsense/", "tests/printsense/"))
    ]
    hits = []
...
def test_5_legacy_marker_allowlist():
    ...
    assert not violations, f"Legacy hashes found in non-allowlist files: {violations}"
...
def test_7_tracked_ignore_rules_are_versioned():
    ...
    assert "printsense/benchmarks/_*/" in dockerignore, (
        "Docker build-context exclusion for confidential eval dirs missing from root .dockerignore"
    )
...
def test_6_draft_labeling_truth_status():
    ...
    assert not violations, f"Invalid truth_status values found: {violations}"
