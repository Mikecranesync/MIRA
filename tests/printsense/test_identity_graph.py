...
def test_02_03_a1_a2_separate_nodes_one_parent():
    ...
    parents = {
        r["to"]
        for r in _rels(g, "CHILD_CONNECTION_POINT_OF")
    ...
    assert not [
        ...
        if "ALIAS" in r["type"] and {r["from"], r["to"]} == {"-21/K01:A1", "-21/K01:A2"}
    ]
...
def test_connector_pin_relationship():
    g = _graph()
    assert ("-XS3:B2", "-XS3") in {(r["from"], r["to"]) for r in _rels(g, "CONNECTOR_PIN_OF")}
...
def test_08_nested_device_only_with_evidence():
    ...
    assert ("-A1-K1" in {r["from"] for r in nested}) or any(
        r["evidence"].get("nested_path") for r in nested
    )
...
def test_10_project_equivalence_requires_confirmed_rule():
    proposed = LegendRule(
        ...
        source_region=None,
        raw_text="K05 == -21/K05",
        mapping={"designation_equivalence": ["K05", "-21/K05"]},
        human_confirmation_status="proposed",
    )
    confirmed = LegendRule(
        source_page="7",
        source_region=None,
        raw_text="K05 == -21/K05",
        mapping={"designation_equivalence": ["K05", "-21/K05"]},
...
def test_every_relationship_states_stage_and_confidence():
    ...
    g = ig.build_identity_graph(index)
    for r in g["relationships"]:
        assert r["stage"] in {"token", "page_context", "project_profile", "revision"}
...
def test_07_alias_count_never_increases():
    ...
    aliases = [
        m for r in report["records"] for m in r["members"] if "ALIAS" in m["typed_relationship"]
    ]
