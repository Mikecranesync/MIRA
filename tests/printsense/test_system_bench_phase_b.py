...
TRUTH = {
    ...
    "grounding": {
        ...
        "protective_functions": {"klixon": "hardwired_trip", "pt100": "analog_monitor"},
    ...
    return [
        {
            "type": "isolation",
            "subject": "-7/K01U",
            "subject_class": "contactor",
            "assertion": "provides_isolation",
            "basis": "observed",
            "cites": ["1"],
        },
        {
            "type": "gating",
            "subject": "-25/V01",
            "subject_class": "thyristor",
            "assertion": "fast_gating",
            "basis": "observed",
            "cites": ["1"],
        },
        {
...
def test_invented_device_costs_identity_precision():
    ...
    cand = _candidate()
    cand["devices"] = [{"tag": "-1/K01", "sheets": ["1"]}, {"tag": "-9/FAKE", "sheets": ["1"]}]
    result = system_bench.score_all(cand, truth)
    assert result["dimensions"]["device_identity"]["score"] == pytest.approx(0.5)
    assert (
        "-9/FAKE"
        in {system_bench._norm(t) for t in result["dimensions"]["device_identity"]["extra_devices"]}
...
def test_g5_invented_rating_topology_state_hard_fail():
    for claim in (
        ...
        },
        {
            "type": "state",
            "subject": "group1",
            "assertion": "regenerating",
            "basis": "observed",
            "cites": ["1"],
        },
    ):
        result = system_bench.score_all(_candidate(claims=_claims_ok() + [claim]), TRUTH)
        assert any(v["rule"] == "G5" for v in _grounding(result)["violations"]), claim
...
def test_5354_drawing_form_plus_conflict_flag_scores_full():
    ...
    assert not any(f["code"] == "convention_conflict_unflagged" for f in result["safety_flags"])
...
def test_5354_correct_form_but_unflagged_conflict_half_credit_plus_flag():
    ...
    assert any(f["code"] == "convention_conflict_unflagged" for f in result["safety_flags"])
...
def test_register_replacement_creates_new_case_never_overwrites():
    ...
    with pytest.raises(ValueError):
        system_bench.register_replacement(
            cases,
            "B5",
            {"kind": "per_page_honesty", "title": "x", "dimensions": []},
            replacement_id="B5R",
        )
...
def test_xref_before_after_compound_string_rescued_with_full_columns():
    ...
    graph = {
        "devices": [
            {"tag": "-1/K01", "type": "contactor", "connects": ["4.4 / X24V.3"]},
        ],
    }
    rubric = {
        "categories": {
...
def test_xref_before_after_expansion_can_surface_misreads():
    ...
    graph = {"devices": [{"tag": "-1/K01", "type": None, "connects": ["157 / X24V.3"]}]}
    rubric = {"categories": {"xref": {"expected": ["X24V.3"], "known_misreads": ["157"]}}}
    result = system_bench.xref_before_after(graph, rubric)
...
def test_permanent_cases_content_is_pinned():
    ...
    canon = json.dumps({k: system_bench.CASES[k] for k in ("B5", "B6")}, sort_keys=True)
...
PINNED_B5_B6_SHA256 = "6da57e7cb2ed1b066c4a6ed471c6a9bd8c9bdf8556d5b4c37ae6b9bc0809a5a1"
