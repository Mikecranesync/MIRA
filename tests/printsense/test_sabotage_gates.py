...
def test_gate_a1_a2_alias_injection_is_detected(monkeypatch):
    ...
    def fake_relate(d1, d2):
        return [{"type": "CONFIRMED_ALIAS_OF", "from": d1.get("raw"), "to": d2.get("raw")}]
...
def test_gate_false_continuity_detected_on_tampered_graph():
                ...
                "devices": [
                    {
                        ...
                        "kind": "x",
                        "ev": "obs",
                        "provenance": {"page_id": "p1", "section": "devices"},
    ...
    tampered["relationships"].append(
        {"type": "CONTACT_MEMBER_OF", "from": "-1/K01:13", "to": "-1/K01:14", "connected_to": True}
    )
...
def test_gate_safety_conflict_survives_bounded_retrieval():
    ...
    many = [{"type": "kind_mismatch", "item": f"-9/Z{i}", "safety": False} for i in range(50)]
    sysgraph = {
        "contradictions": many
        + [
            {
...
def assert_pending_never_frozen(record: dict) -> None:
    if (
        record.get("human_confirmation_status") == "pending"
        and record.get("truth_status") == "frozen_human_confirmed"
    ):
        raise AssertionError("pending human confirmation cannot coexist with a frozen truth")
...
def test_cross_page_bare_tag_merge_is_flagged_not_silent():
    ...
    assert any(
        a.get("kind") == "cross_page_identity_assumed" and a.get("designation") == "-K1"
...
def test_scope_counts_cannot_hide_edges():
    ...
    non_resolved = {
        k: v for k, v in result["scope_counts"].items() if k not in ("resolved", "not_applicable")
    }
