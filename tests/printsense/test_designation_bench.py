...
class TestDesignationBench:
    ...
    def test_benchmark_is_deterministic(self):
        ...
        run1 = json.dumps(designation_bench.run_benchmark(), sort_keys=True)
        run2 = json.dumps(designation_bench.run_benchmark(), sort_keys=True)
        assert run1 == run2
    ...
    def test_report_encodes_metrics(self):
        ...
        result = designation_bench.run_benchmark()
        report = designation_bench.render_report(result)
        for metric_key in ("lexical_accuracy", "false_alias_rate", "raw_preservation"):
            # should appear somewhere in the report
            assert metric_key in report or str(result["metrics"][metric_key]) in report
    ...
    def test_sabotage_detects_alias_violation(self, monkeypatch):
        ...
        assert any("ALIAS" in str(f) or "alias" in str(f).lower() for f in result["hard_failures"])
