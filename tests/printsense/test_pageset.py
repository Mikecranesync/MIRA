...
def _graph_p1() -> dict:
    return {
        ...
        "terminals": [
            {"tag": "-1/X1", "type": "terminal_strip", "confidence": 0.8, "connects": []},
        ],
        "off_page_references": [
            {
                "tag": "FIB1",
                "type": "fiber_continuation",
                "confidence": 0.7,
                "connects": ["sheet3"],
            },
        ],
...
def test_index_has_one_entry_per_manifest_page_never_collapsed():
    ...
    assert [s["quality"] for s in index["sheets"]] == ["clear_upright", "blurred", "missing"]
...
def test_decoder_integration_is_strictly_opt_in():
    ...
    opted = pageset.load_pageset(_manifest(), _graphs(), decoder_profile="eplan_iec")
    s1 = opted["sheets"][0]
    k01 = next(d for d in s1["devices"] if d["tag"] == "-1/K01")
    dz = k01["designation"]
    ...
    graphs["p1"]["devices"].append(
        {"tag": "-1/K01:A1", "type": "coil terminal", "confidence": 0.9, "connects": []}
    )
    opted2 = pageset.load_pageset(_manifest(), graphs, decoder_profile="eplan_iec")
    a1 = next(d for d in opted2["sheets"][0]["devices"] if d["tag"] == "-1/K01:A1")
