...
def _load_text_fixtures(only: str = "") -> list[dict]:
    ...
    for path in candidates:
        ...
        # --only substring filter (2026-08-05): run a targeted slice — e.g.
        # the phone-battery fixtures — without paying for the whole suite.
        if only and only not in path.name and only not in str(fixture.get("tags", "")):
            continue
...
def write_offline_scorecard(
    ...
    # utf-8 explicitly: Windows defaults to cp1252, which cannot encode the
    # scorecard check marks and crashed the writer (0-byte scorecard).
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--only",
        default="",
        help="Substring filter on fixture filename or tags (e.g. 'phone-battery')",
    )
    p.add_argument(
        "--suite",
...
async def _async_main(args: argparse.Namespace) -> int:
    ...
    if suite in ("text", "full"):
        text_fixtures = _load_text_fixtures(only=getattr(args, "only", "") or "")
