...
## v2.34.0 - 2026-09-13
- security(hub): pre-display answer validation gate on the notebook chat route (`NOTEBOOK_ANSWER_GATE`, default ON) — the complete candidate answer is validated before any content byte is released. Unsafe affirmations and bare imperative unsafe instructions (energized-work permitted/instructed, LOTO-declared-unnecessary, safety-device-bypass blessing) are replaced with the shared SAFETY_STOP, ship zero citations, and persist `basis NULL` + a `safety_notice` entry (#3790). General-lane answers may not assert invented fault-code meanings or cite documents the conversation does not have; violations are replaced with a controlled useful fallback (#3787). A Stop before release persists `answerText NULL` — an unvalidated, undisplayed buffer is never stored. Hardened against review findings: clause-scoped honesty exemption, Markdown-normalized detection, imperative/cross-sentence unsafe forms, benign operating-guidance false-positive controls. Iteration-4 hardening replaces head enumeration with a clause-level inversion — any sentence coupling a hazard action to an energized-state or without-isolation relation is unsafe by default unless demonstrably prohibitive/cautionary (catches passive, gerund, polite, and arbitrary-modal phrasings; reassurance idioms never exempt). Iteration-5 hardening binds the prohibitive/cautionary exemption to the clause carrying the hazardous instruction — a warning or negation in an unrelated clause ("There is risk, but …", "Do not hesitate: …") never exempts, while detection stays sentence-scoped so punctuation cannot hide an instruction that spans clauses. Iteration-6 hardening replaces word-presence prohibition with a bound grammar: the negation must reach the hazard verb through auxiliaries only, negate the energized/no-isolation relation directly, or make the hazard the subject of a prohibitive predicate — a negation of some other predicate ("… and not postponed") or an inverted form ("do not hesitate to reset") never exempts. The 2026-09-14 safety-coverage audit implementation extends the floor beyond energized-electrical (polarity-aware prohibition heads; pressurized-work, safety-device-disable, confined-entry, flame-near-gas, under-unsupported-load, rigging-overload, verified-safety-claim, and general-lane exact-setting patterns) and adds the semantic safety layer (#3793): a meaning-aware judge on hazard-flagged candidates through the canonical provider cascade, fail-closed — an unjudgeable flagged candidate is withheld behind a controlled fallback, never silently released (`NOTEBOOK_SEMANTIC_CHECK=0` kill switch). Coverage contract: `docs/specs/safety-coverage-contract.md`.
...
- fix(hub): a disputed asset identity is streamed FIRST on every path (safety stop, abstention, answered) as a marker-only `evidence` frame, so Mobile shows the dispute live exactly as it reloads it.
