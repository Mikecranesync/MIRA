...
export async function GET() {
  ...
  try {
    const { threads, messages } = await withTenantContext(ctx.tenantId, async (c) => {
      ...
      const { rows: tgRows } = await c.query(
        `SELECT username, chat_id, content, timestamp, is_from_bot
      ...
      return {
        threads: rows.map((r: Record<string, unknown>) => ({
        ...
        messages: tgRows.map((m: Record<string, unknown>) => ({
          id: m.chat_id,
          username: m.username,
          content: m.content,
