...
export const config = {
  matcher: [
    ...
    // labs/chat-spike is the DEV-ONLY ChatGPT-class UI compatibility spike
    // (PRD §8.3): the page calls notFound() in production builds, and the
    // fixture transport touches no API route — excluding it lets the spike
    // render in a dev shell without Doppler auth secrets. Remove with the spike.
    "/((?!login|signup|magic|m/|labs/chat-spike|quickstart|api/auth|api/public|api/quickstart|api/health|api/version|api/i3x/|api/scanbe/healthz|api/uploads/folder|api/uploads/[a-f0-9]|_next/static|_next/image|favicon\\.ico|sitemap\\.xml|robots\\.txt).*)",
  ],
};
