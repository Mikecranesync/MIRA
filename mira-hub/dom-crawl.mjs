...
(async () => {
  ...
  for (const url of urls) {
    ...
    await page.goto(url, { waitUntil: 'networkidle', timeout: 30000 }).catch(e => console.error('nav error', e.message));
    const dom = await page.evaluate(DOM_EVAL);

    // Mobile pass
    await page.setViewportSize({ width: 375, height: 812 });
    const mobileConsoleErrors = [];
    page.on('console', m => { if (m.type() === 'error') mobileConsoleErrors.push(m.text()); });
    await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
    const mobileDom = await page.evaluate(DOM_EVAL);
    mobileDom.viewport = 'mobile';
    ...
    results.push({ url, dom, mobileDom, consoleErrors, mobileConsoleErrors, networkFails, focusedEl });
    await page.close();
  }

  await browser.close();
  writeFileSync('/tmp/dom-results.json', JSON.stringify(results, null, 2));
  console.log('DOM crawl complete. Pages:', results.length);
  results.forEach(r => {
    console.log(`\n=== ${r.url} ===`);
    console.log(`  h1_count: ${r.dom.h1_count}`);
    console.log(`  images_no_alt: ${r.dom.images_no_alt.length}`);
    console.log(`  tap_too_small: ${r.dom.tap_targets_too_small.length}`);
    console.log(`  heading_skips: ${JSON.stringify(r.dom.heading_skips)}`);
    console.log(`  og_image: ${r.dom.meta.og_image}`);
    console.log(`  og_url: ${r.dom.meta.og_url}`);
    console.log(`  canonical: ${r.dom.meta.canonical}`);
    console.log(`  twitter_card: ${r.dom.meta.twitter_card}`);
    console.log(`  jsonld_types: ${JSON.stringify(r.dom.jsonld_types)}`);
    ...
    console.log(`  networkFails: ${JSON.stringify(r.networkFails)}`);
    console.log(`  focusedEl: ${JSON.stringify(r.focusedEl)}`);
  });
