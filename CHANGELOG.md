...
### Fixed
- Notebook photo intake is honest end-to-end (Technician Copilot PRD P1, `docs/prd/2026-08-25-technician-copilot-prd.md`): the native image picker no longer ships gallery JPEGs as `application/octet-stream` (the image half of the #3403 picker-mime lie — it caused a 415 rendered as "Couldn't read the nameplate"); both nameplate recognize routes now sniff magic bytes so the bytes decide, not the declared MIME (safelist unchanged — non-image bytes still 415); and the client stops collapsing five distinct failures (415 format / 413 size / 503 recognizer-not-configured / 502 provider / genuinely unreadable plate) into one sentence — each renders its own reason. Also removes the §1.1-violating manual-first empty-state copy (asking never requires setup; sources upgrade answers to cited).
...
### Added
- FactoryLM Fleet Gateway MCP v1 (`fleet-gateway/`): bearer-authenticated control plane with exactly seven tools (`fleet_status`, `task_status`, `launch_worker`, `message_worker`, `request_handoff`, `request_review`, `stop_worker`), mutate audit, hard deny-list, and a loopback-only CAO adapter defaulting to FakeCAO. Public CAO exposure, merge, and deploy are out of scope (#3532). Spec: `docs/specs/fleet-gateway-mcp.md`.
- Follow-up suggestion chips (Copilot PRD **CONV-4**, supersedes #3224): deterministic, evidence-derived next questions (no LLM call, no new retrieval) streamed as an additive `followups` SSE frame and rendered on the LAST answered turn only — web (`NotebookChat`) and phone (`NotebookScreen`); tapping a chip sends it as the next turn through the normal ask path (history included). Never suggests a facet the excerpts did not prove; never on refusals. Also fixes `parseFrame` silently dropping `evidence`/`usage`/`safety` frames on web (the live basis badge was unreachable).
- Durable evidence contract for notebook conversations (Copilot PRD **P2**, migration 084): the evidence-ladder `basis` persists on every served turn — the amber "General guidance" label now survives reload on phone AND web (#3387; never inferred from citation count); `equipment_notebook_sources.origin_file_id` records the canonical photograph behind a materialized nameplate doc, so citation `[1]` and the attachment are two views of ONE object — tapping either opens the actual photo (full-screen, pinch/zoom/double-tap, dependency-free) with the derived text demoted to "Source details"; source rows show a real image thumbnail (conventional glyphs otherwise). Scope contract hardened: an idempotent user_confirmed replay (nameplate confirm retry semantics) no longer silently re-enables a source the user excluded from chat — proven with an 8-case transition truth table on ephemeral Postgres 16 alongside 084 idempotency. Canonical file IDs only; bytes always resolved fresh through the authenticated session; no temporary URLs persisted anywhere.
