...
### Added
- LEGOLAND AWL ingest pack (`tools/legoland/awl_chunks.py`): offline transform turning the T7 drive's 50 plain-text Siemens STEP 7 AWL sources (Chima, Technic Test Track ×2, Aquazone) into citable `knowledge_entries` rows on the ProveIt pattern — fault/alarm glossaries (reusing the `mira-plc-parser` siemens_awl parser, plus a generic commented-STRUCT-member fallback covering German `FltNN : BOOL` fault DBs and `S5TIME` setpoint DBs), and logic-block NETWORK chunks carrying titles, comments, and cabinet-sheet-device symbol references (the print↔PLC join key, e.g. `I_GEN_08_S49`). Rows are `is_private=true`, deterministically id'd (re-runs de-dup), grounded to `enterprise.legoland.site.florida.area.<ride>.equipment.<station>.plc_block.<block>` via the canonical `mira-crawler/ingest/uns.py` builders only. Dry-run CLI (`report`) names files that yield nothing. Real-corpus dry run: 50 files → 259 rows, 3 rides, 3 genuinely content-free files. The proprietary corpus never enters the repo; 13 tests on synthetic fixtures (`tests/legoland/`).
