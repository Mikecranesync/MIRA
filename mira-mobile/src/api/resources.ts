...
export async function getAssetHistory(
  ...
  // A count the server stated, else null. Unavailable windows have no counts.
  const count = (v: unknown): number | null =>
    hc == null || hc.available === false || v == null ? null : Number(v);
  const noCounts = {
  ...
  const historicalCoverage: HistoricalCoverage = hc
    ...
    : reason === "unavailable"
      ...
      : {
          ...
          // an older server never stated admissibility: unknown, not derived
          ...noCounts,
          from: from ?? "",
          to: to ?? "",
