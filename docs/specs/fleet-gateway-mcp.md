...
| `message_worker` | mutate | `text` to one `session_id`. Requires fleet ownership (artifact); unowned IDs raise `OwnershipError` before any CAO call. Allows any fleet-created session, including superseded attempts[]. |
| `request_handoff` | mutate | Write durable HANDOFF artifact; stop claiming the task. Requires fleet ownership AND the session must be the LIVE one (not a superseded attempt). Handoff from a non-live session raises `OwnershipError`. |
