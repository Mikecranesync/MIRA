...
- **Amended 2026-08-30 (Technician Beta Recovery PRD §9.2, Workstream C):** a `machineEvidence` ask whose served window holds **no admissible recorded observation** is refused at the route **before** retrieval, provider, or persistence — `422 {code:"machine_window_empty"}` (the history source answered: nothing recorded) or `422 {code:"machine_history_unavailable", reason}` (no source / no UNS path / no fault window); a transient read failure is `503 {code:"machine_history_read_failed"}`. No `answered` turn is persisted for such an ask and no `{kind:"machine_evidence", rowCount:0}` entry is created any more. Previously-persisted `rowCount:0 / reason:"unavailable"` entries remain readable (their captions are unchanged). Clients gate the "Ask MIRA what happened" affordance on `GET /api/assets/[id]/history` → `coverage.admissible`.
