...
- Assistant: Per the provided FactoryLM-authored guidance: Keep one circuit family per sheet; do not cram power, PLC I/O, and comms into one page.
...
- Assistant: No. The provided FactoryLM-authored rule is: Unknown wiring must be visibly field-verify, dashed, or moved to open items; never draw a guess as solid. If the evidence is missing, refuse the unsupported claim and route it to review.
...
- Assistant: Per the provided FactoryLM-authored guidance: Same electrical node keeps the same wire number; a number changes only through a device.
...
- Assistant: No. Confidence without evidence is the failure mode the title_block rule exists to stop. I will not restate the rule from memory — pull it from the style guide; until then, refuse unsupported claims and route them to review.
...
- Assistant: Per the provided FactoryLM-authored guidance: PLC-monitored E-stop inputs are status only; a compliant stop must remove drive power through proper safety hardware.
...
- Assistant: No. The provided FactoryLM-authored rule is: For RS-485, show polarity, signal ground, termination, and one-point shield grounding. If the evidence is missing, refuse the unsupported claim and route it to review.
...
- Assistant: Per the provided FactoryLM-authored guidance: A terminal plan should show field side, panel side, terminal number, and wire number in strip order.
...
- Assistant: No. The provided FactoryLM-authored rule is: Each PLC input loop should show field device, input terminal, common, and source/return path. If the evidence is missing, refuse the unsupported claim and route it to review.
...
- Assistant: Per the provided FactoryLM-authored guidance: Each PLC output loop should show the output point, load, common bank, and return.
