...
The task brief made several assumptions that the code contradicts. Correcting them up front so we
build against reality, not the brief:

| Brief assumed | Reality in the repo |
|---|---|
| A shared `machine-memory-response.ts` / `buildMachineMemoryResponse` builder | **Does not exist.** The shared lib is `mira-hub/src/lib/machine-memory.ts` (`fetchMachineMemory` + `fetchLiveSignals`). The card response is assembled inline in `machine-memory/route.ts`. |
| An SSE `machine-memory/stream` route + `EventSource` + poll fallback | **Does not exist.** The card **polls** (`setInterval` 2 s for the card, 15 s for sparkline history). No `EventSource` anywhere in `mira-hub/src`. (The SSE/750 ms-poll work referenced in the latency resume was for a *different* card iteration; the shipped `MachineMemoryCard.tsx` is poll-only.) |
| We may need a new `machine_events` table | **Do not build one.** `tag_events` (033) is the raw stream; `machine_state_window` (040) + `machine_run`/`run_diff` (038) already hold state windows + typed anomalies. Migration 040's header explicitly says "This is NOT a machine_events table." |
| `approved_tags` carries tag meaning | **No.** `approved_tags` (035) is the allow/deny boundary only. The tag→meaning map is `tag_entities` (025: `units`, `scaling`, `expected_envelope`, `component_instance_id`) — but it is **not yet wired into the live path**. |

**Net:** the intelligence primitives largely **already exist**. This task is mostly *unify + bridge*,
not *build*.
...
### UI
- **`mira-hub/src/components/MachineMemoryCard.tsx`** — polls `GET /api/assets/{id}/machine-memory`
  every 2 s and `GET /api/assets/{id}/signal-history` every 15 s. Renders: a Run pill, a State pill
...
### Phase 2 — `buildMachineContextPacket(assetId)` (new, `mira-hub/src/lib/machine-context-packet.ts`)
Deterministic, no LLM, returns JSON. **Composes existing pieces** — resolve `uns_path` (kg_entities
bridge) → `fetchMachineMemory` + `fetchLiveSignals` → per-tag `formatTagValue` +
`classifyTagFreshness` + `changed_recently` (from `last_changed_at`) → `deriveCurrentState`. Packet:
```
{
