...
| `OCR_MODEL_LANE` | mira-bot-telegram — model-OCR enrichment lane via the inference cascade. Values: `off` (default — uses free Tesseract floor only) or `on` (adds model inference cost). The deterministic Tesseract floor runs in-image; model lane is benchmarked separately. Keep off unless benchmarked. Containers: default off. |
| `OCR_EXPECT_TESSERACT` | mira-bot-telegram — boot self-check Tesseract floor status. Values: `1` (containers/prod — floor required, self-check reports DEAD if missing) or `0` (local dev/Windows machines — floor optional). Default: `1` in `docker-compose.staging-vps.yml` + `docker-compose.saas.yml`, `0` in `mira-bots/docker-compose.yml`. |
