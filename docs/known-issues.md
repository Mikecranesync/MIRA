...
- **Nameplate photo → unknown drive still has no internet manual-search fallback on the Telegram fast-path (partially updated 2026-08-31 — see correction below)** — `_try_nameplate_drive_pack_reply` (`mira-bots/telegram/bot.py:1281-1389`) matches a photographed nameplate against only the 3 shipped Drive Packs (GS10, PowerFlex 525, PowerFlex 40). On a miss it returns `False` and falls through to the generic engine FSM instead of trying to find the official OEM manual — the technician gets a generic troubleshooting reply for a drive MIRA could not identify. **This core gap is still real** — `grep -rln "manual_search" mira-bots/telegram/` returns nothing.

  **⚠️ CORRECTION — the rest of this entry was stale.** The previous version said `manual_search.py` "was never connected to the main bot" and called the whole thing "drift, not a scope decision." That's no longer accurate: `mira-scan-monday/backend/manual_search.py` **was ported** into `mira-bots/shared/manual_search/` (PR #3042, merged 2026-08-02), and that shared module has since seen active follow-on work — `mira-bots/shared/manual_search/judge.py` (PR #3411, "model-judged discovery," merged 2026-08-26) and PR #3401 (manual-confirm download arbiter, merged 2026-08-25). It's genuinely wired today into `equipment.default_manual_retriever()` (Phase 2 of the plan below, ladder rungs 4-6, behind `MIRA_MANUAL_SENSE_WEB_ENABLED`, **default off**) and into `mira-bots/ask_api/manual_discovery.py` (a Hub-facing API). The module's own docstring (`mira-bots/shared/manual_search/__init__.py`) states plainly: "Phase 3 (connecting a bot adapter's pack-miss path to this) is separate and not done here" — this is **tracked, phased, actively-developed work with its own governing plan**, not unowned drift. See `docs/plans/2026-07-31-visual-intake-asset-identity-manualsense-audit.md` for the phase breakdown. Given the density of recent related merges (3 PRs in the last month, one 5 days old at time of correction), treat this as **live territory** — check for an active claim before touching Phase 3, don't assume it's unowned.
...
- **Gemini key blocked → RESOLVED by removal.** `GEMINI_API_KEY` returned 403 in Doppler; Gemini was then dropped from the product cascade entirely and **Together** became the third provider (`TOGETHERAI_API_KEY`; `AGENTS.md` § Hard constraints — Gemini is banned). Do not provision a Gemini key for the product. CI review/judge workflows keep their own fallback lists in their `.yml`.
...
- ~~`tools/demo_plc_poller.py` ships a colliding `live_signal_cache` DDL~~ — **FIXED**. The poller no longer defines its own `SCHEMA_DDL` or writes to NeonDB directly (that embedded DDL, keyed on `topic`, never matched Hub migration `020`'s `(tenant_id, plc_tag, …)` shape). Per `.claude/rules/one-pipeline-ingest.md`, it now builds tag entries via `mira-relay/ingest_contract.py` (`build_tag_entry`/`build_ingest_batch`, loaded by file path like `simlab/publishers.py::RelayIngestPublisher`) and posts to `POST /api/v1/tags/ingest`; the relay's `tag_ingest.ingest_batch` (migrations 020/033/036) is the only NeonDB writer. Surfaced 2026-05-29, fixed on `audit/poller-schema`.
...
| Anthropic / Claude (banned) as cloud LLM provider | Groq → Cerebras → Together cascade (Gemini, the third leg at the time of #610, is now banned too) | Removed PR #610. Do not reintroduce. |
