...
**Prior findings:** 4 · **Rulings:** 4 (sustained: 2)
...
- diff chars sent/total: 151,078/151,078 (cap 200,000)
- reviewed-diff sha256 (sent bytes): `a22bcd7ca395c06ca09b7eeaefadf12a21354e336bd8c41a24711fceb9667966`
- full scoped-diff sha256 (pre-cap): `a22bcd7ca395c06ca09b7eeaefadf12a21354e336bd8c41a24711fceb9667966`
...
- F1 [high] Inclusion of `.log` in documentation suffixes creates a data‑leak risk
- F2 [high] `is_evidence_artifact` incorrectly excludes all files under `units/evidence/` from review
- F3 [high] Parser regression – malformed findings/rulings are now silently ignored
- F4 [medium] `canonical_source_url` does not normalise default ports, enabling duplicate rows

## Rulings

- **[SUSTAINED] F1** [high] Inclusion of `.log` in documentation suffixes creates a data‑leak risk
- **[REFUTED] F2** [high] `is_evidence_artifact` incorrectly excludes all files under `units/evidence/` from review
- **[REFUTED] F3** [high] Parser regression – malformed findings/rulings are now silently ignored
- **[SUSTAINED] F4** [medium] `canonical_source_url` does not normalise default ports, enabling duplicate rows
