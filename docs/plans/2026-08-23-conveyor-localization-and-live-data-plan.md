...
- `…/[id]/chat/route.ts` — call after `validateChatSources`.
  ...
  - **The 422 body must be technician-readable.** Mobile's `askNotebook` → `errorFromStatus(422, res.data)` → `kind='client'`, `detail = data.error` → guarded `apiErrorCopy(error)` renders that detail. So `{"error":"uns_required"}` renders on Mike's phone as the literal token `uns_required`. Return `{"error":"<plain sentence>", "code":"uns_required", "notebookId":…, "entityId":…}` instead, keeping the discriminator in `code`. The same trap applies to I4's 422s through `TagLanding`.
