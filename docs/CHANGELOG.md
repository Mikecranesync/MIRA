...
### v3.25.0 (2026-06-16) — MIRA PLC Parser Phase 1: read-only export analysis + offline CLI (PR #2062)
- **`mira-plc-parser/`** — new isolated, stdlib-only subproject. Read-only, vendor-agnostic PLC program-export analysis: detect → parse → MIRA PLC IR → deterministic maintenance intelligence (tags, routines, output-dependency map, fault/asset/VFD-signal candidates, safety **review** flags). Parser-first — not a PLC writer, not vendor-to-vendor translation, not safety validation. Closed/binary vendor project files (`.ACD`/`.ap1x`/`.s7p`/`.project`/archives) are rejected with precise export instructions.
- **`mira-plc-parser/mira_plc_parser/cli.py` + `__main__.py`** — Phase 1 DoD: offline CLI `mira-plc-parser analyze <FILE> [--out] [--format md|json|both]` writes local `<stem>.report.md` / `<stem>.report.json`. Stdlib-only (no LLM, no network). Exit codes: 0 parsed · 3 closed vendor project file · 1 unrecognized/read error. `pipeline.render_json()` is the json.dumps-safe sibling of `render_markdown()`.
- **`mira-plc-parser/PACKAGING.md` + `mira-plc-parser.spec`** — standalone Windows `.exe` via PyInstaller (stdlib-only ⇒ no hidden imports; `.exe` not committed).
- **`ignition/webdev/FactoryLM/api/diagnose/tag_csv.py`** — vendor-agnostic CSV tag-export engine reused by the parser's CSV path (included as its runtime dependency).
- **Tests:** 38 (fixture-based; CLI + report generation covered). `python -m pytest mira-plc-parser/tests/` green, ruff clean. Not yet wired into Hub/MCP — later phase.
