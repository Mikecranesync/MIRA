...
5. **Verify on production.** Smoke the affected route (`bash install/smoke_test.sh`). For UI: use `docs/runbooks/visual-proof.md`. For bots: send a real message to `@FactoryLM_Diagnose` and inspect the reply against grounding rules.
