...
2. **Phase 2 — grounding** (cite the factory's own data): Pilot DB JSON (`Enterprise B/Pilot
   Database Export/`) → human-readable citable chunks (Item→Lot→WO→Asset + State) and a local manual
   PDF → `knowledge_entries` + embeddings (the **only** citable path — GUI/OW KB is non-citable; see
   `knowledge-entries-tenant-scoping.md`). **Needs:** NeonDB + embeddings (infra) and a **real
   beverage-bottling manual PDF** (none in the corpus). The *offline transform* half is buildable
   without infra and is the best next no-infra increment.
...
## Exact next steps
- **No-infra:** build the Phase-2 **Pilot DB → citable chunks** offline transform (pure function +
  synthetic fixture + tests), so the embed/insert step is the only infra-gated remainder.
