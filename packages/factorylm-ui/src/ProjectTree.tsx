...
export function ProjectTree({ projects, state, dispatch, onOpenItem }: ProjectTreeProps) {
  ...
  return <ul className="fl-tree" aria-label="Projects">
    {projects.map((project) => (
      <li key={project.id}>
        ...
        <Nodes nodes={project.children} state={state} dispatch={dispatch} onOpenItem={onOpenItem} />
      </li>
    ))}
  </ul>;
}

function Nodes({ nodes, state, dispatch, onOpenItem }: Omit<ProjectTreeProps, "projects"> & { readonly nodes: readonly ProjectNode[] }) {
    ...
    {nodes.map((node) => {
      if (node.kind === "folder") {
        return <li key={node.id}>
          ...
          <Nodes nodes={node.children} state={state} dispatch={dispatch} onOpenItem={onOpenItem} />
        </li>;
      }
      ...
      if (onOpenItem) {
        return <li key={node.id}>
          <button
            type="button"
            className="fl-tree__item-button"
            data-item-id={node.id}
            data-item-kind={node.kind}
            aria-current={state.thread.id === node.id ? "page" : undefined}
            onClick={() => {
              onOpenItem(node);
