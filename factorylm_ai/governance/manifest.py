...
def _entry(record: dict) -> dict:
    return {k: record.get(k) for k in _ENTRY_FIELDS}
...
def corpus_manifest(records: list[dict], *, dataset_version: str) -> dict:
    ...
    sorted by ``record_id`` before the digest, so the same inputs (in any order)
    yield the same ``manifest_sha256``."""
    entries = sorted((_entry(r) for r in records), key=lambda e: str(e.get("record_id")))
    ...
    for e in entries:
        ...
        if s in split_counts:
            split_counts[s] += 1
        if e.get("document_lineage_key"):
            lineages.add(e["document_lineage_key"])
