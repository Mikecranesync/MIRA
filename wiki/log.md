...
## [2026-08-18] session | Charlie — channel-neutral manual/Files workflow draft #3300
- Claimed #3299 and implemented the Hub-owned normalized operation/workspace boundary used by Telegram and Slack, reusing #3245 nameplate/manual/Files/notebook seams instead of adding another bot-local finder. Preserved the complete Danfoss FC-202 floor incident as a regression fixture.
- Added tenant RLS, event fingerprint idempotency, execution fencing, at-most-once terminal claim/ACK, canonical session/notebook/File/document links, reset rotation, official-OEM/applicability gates, grounded citations, adapter parity, and fail-closed deployment validation behind an off-by-default flag.
- Adversarial round 1 at `099e4416d` found 2 high/1 medium/1 low. Fixed the reset crash after rotation using a unique reset-operation witness; rejected non-atomic attachment sets before allocation; aligned schema/runtime UUID boundaries and removed bot text truncation. Kept one-shot delivery because reclaiming an uncertain transport send violates the explicit no-duplicate terminal invariant; added observable `claimed_unacknowledged` state.
- Verification after remediation: Hub 1,993/1,993; security/deploy/review 148/148; build, migration order, changed-file ESLint/Ruff/Prettier green. Bot run: 2,499 pass/20 skip plus two untouched wiring-intake test failures reproduced identically on a detached `origin/main` worktree (byte-identical files).
- No merge/deploy/migration/secret mutation. Live Telegram plus Hub/mobile Danfoss replay remains `PENDING-HUMAN` for Mike GO and staging configuration. Latest exact-SHA review and CI authority lives on draft PR #3300.
- Machine: Charlie
