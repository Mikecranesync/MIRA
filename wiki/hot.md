...
## eval-fixer run — 2026-07-29
- **Evaluated commit: `f1da01fe` (main)** — established from reflog (HEAD unchanged since 07-28 22:12, clean tree on main at the 01:23 run); scorecard still doesn't stamp a SHA (#2952).
- Scorecard: **43/57 (75%)** — `tests/eval/runs/2026-07-29T0123-offline-text.md`. 6 patchable / 8 need human review; watchdog reports no regressions vs prior run.
- Action: **issue-filed** (comment on rolling tracker #1876). No patch — the 3-key `file_clusters` hard stop fired again (engine.py ×5, guardrails.py ×1, active.yaml ×1; #2759 unchanged).
- `main` band is now n=3: 35, 36 → **43**. +7 over yesterday with no engine change shipped in between — consistent with the documented ~15pt suite swing, not evidence of improvement.
- Yesterday's Cluster 1 (identical generic clarification turn) shrank 7 → 2 fixtures (`sew_overcurrent_29`, `vfd_abb_03_acs355_cross_load`) — supports the swing/nondeterminism reading.
- **Lead cluster tonight: wrong-vendor citations — 8/14 failures** (`cp_citation_vendor_relevance`, skip_reason "Unknown"): Rockwell/Yaskawa/Siemens chunks cited for AutomationDirect/SEW/Lenze/Danfoss/Siemens assets. Yesterday's fail-open caveat stands — these fire because retrieval now *returns* (wrong) chunks. Fix surface is retrieval vendor-scoping (`recall_knowledge`, cf. #2083/#2085), unreachable from the 4 allowed patch files. Note `vfd_siemens_04` shows the reply layer correctly *refusing* the Rockwell chunks it was handed.
- FSM `cp_reached_state` cluster: 5 fixtures → engine.py — 3 stuck one Q short (Q1→Q2 / Q2→DIAGNOSIS advance), 2 fell to IDLE (intent routing to the history/summary path instead of the FSM).
