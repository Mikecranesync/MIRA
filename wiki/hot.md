...
- 🔴 **NEW — the dominant cluster has a direction nobody had named: wrong-vendor citation is 8/15, and it is entirely long-tail→big-corpus.** AutomationDirect ×3, Danfoss ×3, SEW ×1, Lenze ×1 — all answered with Rockwell/Yaskawa/Siemens sources. 100% of failing assets are long-tail; 100% of cited vendors are the three largest corpora. Hypothesis: generic fault language ("overcurrent", "thermal") lexically matches the Rockwell/Yaskawa-heavy shared corpus, so retrieval falls back to **corpus mass instead of vendor scope**, and does not fail closed when the long-tail vendor is thin. Cleanest exhibit — `lenze_thermal_30`: *"the **Lenze i550** drive [Source: **Yaskawa V1000** — Speed (%)]"* in one sentence. `gs1_12`/`gs3_14` are worse — `PowerFlex`/`Rockwell` leak into the **prose** of an AutomationDirect answer. Same family as **#2083**, and customer-facing. **This is a retrieval-precision bug, not eval tuning** — entry point is `recall_knowledge` via the `retrieval-diagnostics` skill, not the harness.
