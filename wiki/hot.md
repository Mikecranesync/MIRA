...
## eval-fixer run — 2026-07-31
- **Evaluated commit: `858da586`** — branch `codex/charlie-document-evidence-compiler`, **not `main`** (4 behind / 8 ahead). Scorecard validity was **verified both directions** before trusting it: `git diff --name-only origin/main...HEAD` (our 8 commits) and `HEAD...origin/main` (main's 4) each touch **zero** eval-surface files. `engine.py`, `guardrails.py`, `prompts/diagnose/`, `tests/eval/` are byte-identical to `main`, so this **is** an honest measurement of shipped code. (Contrast the 465-commit staleness window — nothing like it here.)
- Scorecard: **42/57 (74%)** — `tests/eval/runs/2026-07-31T0404-offline-text.md`. 15 failures: 8 "patchable" / 7 need human review. `"regressions": []`.
- Action: **issue-filed** (comment on rolling tracker #1876). No patch. Steps 5–7 (baseline + verify `offline_run.py`) skipped deliberately per the no-patch path — two ~37-min live-inference runs cost real budget and prove nothing.
- 📊 **Honest `main` series is now n=4: 35, 36 (`c89086ef`) → 39 (`84b334bb`) → 42 (`858da586`).** Record 42 as a data point, **not** as improvement — the documented ~15pt swing still applies, and tonight had **2** timeout phantoms vs 07-30's **4**, so ~2pt of the 39→42 delta is just fewer provider stalls.
- **Cause 1 — cross-vendor citation contamination, 9 fixtures (60%, the dominant real cause).** `gs1_12`, `gs3_14`, `pf520_17`, `sew_29`, `lenze_30`, `vfd_abb_01`, `vfd_abb_03`, `vfd_danfoss_04`, `vfd_siemens_03`. Rockwell/PowerFlex is the wrong-side vendor in 6 of 9 — corpus skew, cf. #2083/#2085. `gs1_12` and `gs3_14` trip **two independent checkpoints** (forbidden-keyword *and* citation-vendor) on one root cause, which corroborates the diagnosis. **Unreachable from any of the 4 allowed patch files.** ⚠️ 9 is a **floor, not a count**: `cp_citation_vendor_relevance` is still fail-open (`grader.py:376-407`), so "stop citing anything" would score as a win.
- **Cause 2 — FSM pacing, 4 fixtures, still internally contradictory.** `pf525_02` needs **fewer** questions (Q2→DIAGNOSIS); `asset_change_08` (Q1→Q2) and `vfd_danfoss_03` (Q2→Q3) need **more**; `self_critique_34` never entered the FSM (IDLE→Q1). Re-confirms the 07-30 correction: no single skip-logic change satisfies that set. `engine.py` is the largest *single-file* cluster and is exactly what the spec's "pick the biggest cluster" line would select — it must not be. **Product decision, not a patch.**
- **Cause 3 — timeout phantoms, 2 fixtures** (`gs10_01`, `danfoss_27`), both rendering `fallback_responses.py:40` prose. **Failure 5 is unfixed:** `tests/eval/` and the eval workflows still never set `MIRA_PROCESS_TIMEOUT` (verified by grep) → inherits **30s** (`engine.py:365`), while Slack ships **60s** and the mira-ask kiosk **90s**. The eval remains the strictest latency surface in the repo.
- 🔴 **New evidence sharpening #2759 — the gate misfiles causes, it doesn't merely over-trigger.** Hard-stopped again on 3 `file_clusters` keys (**21+ nights, still zero patches ever**). But this run shows it fixture-by-fixture: **all 4 "autopatch-eligible" `cp_keyword_match` failures are misfiled — 2 are timeout phantoms (`gs10_01`, `danfoss_27`) and 2 are cross-vendor contamination (`pf520_17`, `vfd_abb_03`). Zero are genuine missing-keyword / honesty-signal gaps.** So even on a run where the gate *had* fired, `guardrails.py` would have been the wrong file and the patch a **guaranteed no-op**. That is a stronger argument for fixing/retiring the gate than "3 clusters again".
- ⚠️ **Failure 4's "correct-by-configuration, not by construction" warning has recurred.** The shared tree is on `codex/charlie-document-evidence-compiler`, so `safe-cron-pull.sh` skip-condition #4 (*not on main*) means **tomorrow's nightly will no-op the pull and grade whatever this branch has drifted to** — silently, with no parity check unless the agent runs one. Tonight survived only because the drift happened to miss the eval surface. I did **not** check out `main` to "fix" it — that branch is another session's in-flight work. **#2952 (dedicated `main`-pinned worktree + SHA stamping) is the real fix and is still open.**
- Suggested order for a human: **#2952** (recurred tonight, caught only by a manual check) → **#2759** (now demonstrably misfiling, not just over-triggering) → set `MIRA_PROCESS_TIMEOUT=60` in the harness (expect ~4–7pt of noise removal — **do not read it as a code improvement**) → cross-vendor retrieval precision, but fix the fail-open grader first or the fix can't be measured.
