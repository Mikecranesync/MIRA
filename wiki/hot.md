...
- ⚠️ **The timeout confound is itself NEW — check infra first.** Placeholder count went 2 (07-22) → **14 (07-25)** → **10 (07-26)**; runtime 2170s (07-17) → 2987s (07-26). `git log --since=2026-07-21` shows **no commits touching `mira-bots/` or `tests/eval/`** in that window ⇒ not an engine/harness code change. Points at provider/model/infra latency (cascade routing, model swap, rate-limit backoff) as the first thing to check.
- ⚠️ **Underlying pass rate is also sliding (normalized):** a timed-out fixture is removed from the pool that can fail for real reasons, so raw counts mislead. Non-timeout failure rate **12/43 (28%) on 07-25 → 20/47 (43%) on 07-26**. Watchdog flagged 0 regressions; worth a human look independent of the timeout confound.
...
- 🚨 **BIGGEST FINDING — this scorecard does not measure `main`.** The eval runs from the working tree on `fix/precommit-untracked-sigpipe`, which is **444 commits behind `origin/main`, 122 of them touching `mira-bots/`**. The 47% pass rate, the FSM-pacing cluster and the "degradation" trend all describe a months-old snapshot of the engine, not shipped code — some of those ~12 FSM failures may already be fixed on main. Main's 07-21→07-26 window includes the ADR-0031 hardening series (#2861 provider registry + network gate, #2862 Together/MiniMax typed interpreter, #2863 required-OCR) and #2913 RAG grounding hardening. **Rebase the eval checkout onto main (or run the nightly from main) before acting on any cluster above** — that's a bigger lever than any patch this agent could apply.
- Timeout precedent: #1632 (merged) already raised `MIRA_PROCESS_TIMEOUT` to 90s for the mira-ask kiosk ⇒ 30s is known-tight and there's precedent for raising it **per-surface via env**. If timeouts persist post-rebase, set it in the *eval harness environment* — still not an `engine.py` edit.
