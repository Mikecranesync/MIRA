...
- Action: issue-filed (comment on rolling tracker #1876 — no new issue)
- **The 27→49 recovery is mostly the timeout confound clearing, not a code fix.** Tonight's scorecard contains **zero** `TIMEOUT_WARNING` failures (07-26 had 10, 07-25 had 14) and runtime fell 2987s → 1992s. Non-timeout failures did also improve genuinely (20 → 8), so the last two nights' "degradation" was largely provider latency, not a regression. No code changed in between — reinforces that single-run deltas can't distinguish a fix from noise.
