...
## eval-fixer run — 2026-08-03
- Scorecard: `2026-08-03T0049-offline-text.md` — **41/57 (72%)**; 16 failures (9 "patchable", 7 skip). Graded SHA **`6ad2e622`**.
- Action: **issue-filed** (no patch). 3 file clusters vs a 1-cluster gate — #2759 again, ~11th consecutive no-op night. Report → #1876; two long-stranded causes finally filed as **#3085** + **#3086**; sharpened misfiling evidence → #2759; provenance evidence → #2952.
- 📌 **PROVENANCE — 08-02's open caveat is CLOSED, and the answer is not the obvious one.** The scorecard filename is **UTC, computed at _write_ time** (`tests/eval/offline_run.py:365`, inside `write_offline_scorecard`) — so `T0049` is the run's **end**, not its start. This run finished **2026-08-02 20:49 EDT** and, at 2010.3 s, started ~20:16 EDT. ⚠️ **The naive read is wrong**: the tree's current HEAD (`d875b6f7`) and its two siblings (`51bbc686`, `04417504`) were committed 00:21–00:33 EDT on 08-03 — i.e. **~4 h AFTER the eval finished** — and touch only workflows + compose. Graded HEAD is **`6ad2e622`** (`fix(spine)… (#3067)`, 20:14:23 EDT, 1.5 min before the run began), which **is an ancestor of `origin/main`**. First time the graded SHA has been established as fact rather than assumed. Method: `stat` the scorecard mtime, confirm `run_date` is computed at write time, subtract runtime, then `git log --before`.
- 🔴 **NEW VARIANT of Failure 4 — `main` outran the graded commit within hours, on exactly the eval surface.** `git diff 6ad2e622..origin/main` touches `mira-bots/shared/engine.py` (**+288**) and `guardrails.py` (**+129**) — 410 insertions across #3068, #3069, #3072, **#3074 (routing gauntlet + safety-keyword sweep)**, #3077. Those are the routing/FSM/guardrails surfaces that dominate tonight's taxonomy. This is *not* "graded a stale branch" (the graded commit was on `main` at run time) — it is **"graded a commit that `main` outran within hours."** ⚠️ **Do not act on tonight's FSM or keyword clusters without re-running at current `main`.** This is the concrete case #2952 needs: stamping the SHA into the scorecard footer would have made it a one-line check instead of a four-command reconstruction from file mtimes.
- **Cause 1 (joint lead) — cross-vendor citation bleed, 7 fixtures.** `gs1_12`, `gs3_14`, `sew_29`, `lenze_30`, `vfd_danfoss_01`, `vfd_danfoss_04`, `vfd_siemens_03`. **Exactly the 7 `skip_failures`** — one root cause covers every failure autopatch may not touch. Same long-tail→big-corpus directionality already quantified on **#3049**; not new, tracked. 🚧 Standing blocker unchanged: `cp_citation_vendor_relevance` is **fail-open** (`tests/eval/grader.py:376-407`) — fix the grader *before* touching retrieval, or a "cite nothing" regression scores as a win.
- **Cause 2 (joint lead) — FSM pacing, 7 fixtures, contradictory on 3 verified nights (07-30, 07-31, 08-03) → FILED as #3086.** ⚠️ Corrected from a first draft that claimed "5th consecutive night" — 08-01/08-02 were inferred, not checked for this cluster (correction posted to #3086/#1876). Also note `sew_29`/`vfd_danfoss_04` end on a clarification question but fail only `cp_citation_vendor_relevance`, **not** `cp_keyword_match` — one engine behaviour, two checkpoint verdicts, which sharpens rather than weakens the #2759 argument. Deliberately **not** framed as growth from 07-30's 4: that's n=1 vs n=1 inside the documented ~15pt swing, and `cmms_32`'s bucket is soft. The solid part is the direction split: **1 needs fewer** (`pf525_02` Q2→DIAGNOSIS), **4 need more** (`vague_05`, `asset_change_08`, `pf527_20`, `vfd_danfoss_03`), **1 never entered** (`self_critique_34` IDLE→Q1), **1 stuck in revision** (`cmms_32` DIAGNOSIS_REVISION→RESOLVED). No single skip-logic change satisfies that set. `engine.py` is exactly what the spec's "biggest single-file cluster" line would wrongly select — **product decision, not a patch.**
- **Cause 3 — non-answer endings, 5 fixtures. A SECOND mechanism, now named.** Beyond the 2 known timeout phantoms, **3 more fixtures end on a clarification question rather than an answer** and are graded as keyword misses. Prose-grep of the scorecard (grep the rendered prose, never the constant — 07-27 method note): `Before I can give you a confident diagnosis` ×3 (`sew_29`, `lenze_30`, `cmms_32`), `taking longer than usual` ×2 (`gs10_01`, `vfd_abb_03`), `To better understand this issue` ×1 (`vfd_danfoss_01`), `Can you check the alarm code` ×1 (`vfd_danfoss_04`).
- 🔴 **Sharpest #2759 argument yet: ZERO genuine missing-phrase failures.** All **7** `cp_keyword_match` failures decompose without a single missing phrase in `guardrails.py`/`active.yaml`: **2 timeout** (`gs10_01`, `vfd_abb_03`) + **3 clarification-ending** (`lenze_30`, `cmms_32`, `vfd_danfoss_01`) + **2 forbidden-keyword = cross-vendor bleed into prose** (`gs1_12`, `gs3_14`). So even on a night the gate *had* fired, `guardrails.py` would be the wrong file and the patch a **guaranteed no-op**. Stronger than 07-31's version because the *second* non-answer mechanism is now named, not just timeouts. Proposed fix specified on #2759: build `file_clusters` from an **observed-response diagnosis**, not `CHECKPOINT_META`'s static map.
- ♻️ **Failure 5 confirmed unfixed, 5th night → FILED as #3085.** `grep -rn "MIRA_PROCESS_TIMEOUT" tests/ .github/workflows/` returns **only** the two Slack-contract assertions; the eval inherits the **30 s** default (`engine.py:368`, enforced `:1891`) vs Slack **60 s** (`test_slack_deploy_contract.py:30`) and kiosk **90 s** (#1632). It had only ever been cross-posted to #2759, a different bug. ⚠️ When the config fix lands expect a ~2–7pt jump — **that is the instrument changing, not the code improving.**
- **Honest `main` series: `35, 36` (`c89086ef`) → `39` (`84b334bb`) → `42` (`858da586`) → `42` (08-02) → `41` (`6ad2e622`).** 41 is **in band, not a regression**; ~2pt of it is timeout phantoms. The ~15pt swing still means no single run is a trend.
- 🧭 **Method notes that paid off tonight** — (a) `grep -n "Cause 1" wiki/hot.md` before writing, per [[feedback_hot_md_tail_hides_prior_causes]]: it correctly stopped a "new finding" framing on cross-vendor bleed *and* surfaced the fail-open-grader blocker. (b) Parity verified **both** directions before trusting an off-`main` scorecard (07-31 technique) — but note it must be run against the **graded** SHA, not current HEAD; checking current HEAD alone would have missed the 410-line drift entirely. (c) Spec **Steps 5–7 skipped** on this no-patch path (two full `offline_run.py` passes cost real budget and prove nothing), and **Step 10 overridden** — wiki lands via worktree→PR, never a direct `main` commit.
