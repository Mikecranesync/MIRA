...
from .flight_report import write_report
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--html", metavar="PATH",
                    help="render the Flight Recorder Report to a self-contained HTML file at PATH")
    args = ap.parse_args(argv)

    result = run_pipeline(scenario=args.scenario, seed=args.seed, live=args.live)
    if args.html:
        path = write_report(result, args.html)
        print("Flight Recorder Report written to: %s" % path)
        print("Open it in any browser (double-click) — no server, no login, no code needed.")
    elif args.json:
