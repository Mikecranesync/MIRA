...
## Visual report (for non-programmers) — the Flight Recorder Report

Generate a **self-contained HTML report** you open in a browser — no server, no login,
no code, no cloud. This is the human-readable "black box readout" of one run.

```bash
# scenario A → an HTML file you double-click
python -m demo.factory_difference_engine --scenario A --html Flight-Recorder-Report.html
```

- **Output path:** whatever you pass to `--html` (e.g. `Flight-Recorder-Report.html` in the
  current folder). The command prints the exact path it wrote.
- **Open it:** double-click the file → it opens in any browser (Chrome/Edge/Safari). Everything
  (styling included) is inline; nothing is fetched from the internet.

**What you should see** (top to bottom):
1. **Header** — Northwind Bottling / CV-200, scenario, seed, "deterministic replay".
2. **Executive Summary** — a plain-English paragraph (what changed, likely cause, what to check
   first) + a metrics strip (signals discovered, tags approved, manuals, **PLC writes = 0**).
3. **Difference Cards** — one card per detected difference (signal, normal vs current, how far off).
4. **Event Timeline** — the differences grouped onto a simple time axis as one machine event.
5. **Baseline vs Current** — a small bar per signal showing the normal band and the current value.
6. **Explanation** — MIRA's maintenance-facing answer + recommended checks (with a rubric score).
7. **Evidence & Citations** — the abnormal PLC signals and the cited manuals.
8. **Learn / Review Preview** — proposed context updates with accept / reject / escalate (static preview).
9. **Caveats** — what's still a demo alias / unproven, and the scenario+seed for reproducibility.

Regenerating with the same `--scenario` and default seed produces a **byte-identical** report.
