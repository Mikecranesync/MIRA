...
MAX_PDF_PAGES = 25
STATUSES = ("received", "queued", "processing", "needs_review", "delivered", "failed")
...
def _safe_display_name(name: str) -> str:
    # PureWindowsPath treats BOTH / and \ as separators on every host OS;
    # plain Path(name).name keeps backslash directories on POSIX, letting
    # "evil\..\name.png" survive with its dots. Collapse any leftover ".."
    # runs so the display name can never look like a traversal.
    leaf = PureWindowsPath(name).name
    base = re.sub(r"\.{2,}", "_", re.sub(r"[^A-Za-z0-9._ -]", "_", leaf))[:80]
    return base or "upload"


def submit_intake(
    req: IntakeRequest, files: list[tuple[str, bytes]], root: str | Path, tenant_id: str
) -> dict:
    if not files:
        raise IntakeRefused("at least one file is required")
    if len(files) > MAX_FILES:
        raise IntakeRefused(
            f"at most {MAX_FILES} files on this surface — request the managed package pilot instead"
        )
    total = sum(len(d) for _, d in files)
    if total > MAX_TOTAL_BYTES:
        raise IntakeRefused(
            "upload too large for this surface — request the managed package pilot instead"
        )
    stored = []
    ...
    intake_id = sha256_bytes((req.work_email + "|" + str(time.time_ns())).encode())[:20]
    ...
    for name, data, kind in stored:
        ...
        logger.info("intake=%s tenant=%s file sha=%s kind=%s", intake_id, tenant_id, sha[:12], kind)
    record = {
        "intake_id": intake_id,
        "tenant_id": tenant_id,
        "status": "received",
...
def get_intake(root, tenant_id: str, intake_id: str) -> dict:
    return json.loads(_record_path(root, tenant_id, intake_id).read_text(encoding="utf-8"))


def set_status(root, tenant_id: str, intake_id: str, status: str, note: str | None = None) -> dict:
    if status not in STATUSES:
        raise ValueError(f"unknown status {status!r}")
    p = _record_path(root, tenant_id, intake_id)
    rec = json.loads(p.read_text(encoding="utf-8"))
    rec["status"] = status
    rec["history"].append({"at": time.time(), "status": status, **({"note": note} if note else {})})
