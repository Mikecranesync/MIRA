...
def build_system_graph(index: dict) -> dict:
    ...
    for s in sheets:
        ...
        for d in s.get("devices", []):
            ...
            if ev == "obs" and not observable:
                violations.append({"code": "phantom_observation", "sheet": sid, "item": d["tag"]})
                continue
            key = _norm(d["tag"])
            node = devices.setdefault(
                key,
                {"tag": d["tag"], "kinds": set(), "sheets": set(), "ev": ev, "first_sheet": sid},
            )
        ...
        for x in s.get("xrefs", []):
            ev = x.get("ev", "obs")
            if ev == "obs" and not observable:
                violations.append({"code": "phantom_observation", "sheet": sid, "item": x["sig"]})
                continue
            peer = str(x.get("peer", ""))
    ...
    def _is_safety(*subjects: str) -> bool:
        return any(_norm(_head(s)) in safety_set or _norm(s) in safety_set for s in subjects if s)
    ...
    for node in devices.values():
            ...
            violations.append(
                {
                    ...
                    "sheet": node["first_sheet"],
                    "item": node["tag"],
                    "expected_sheet": prefix,
        ...
        if len(kinds) > 1:
            violations.append(
                {"code": "duplicate_conflict", "item": node["tag"], "kinds": sorted(kinds)}
            )
    ...
    for s in sheets:
        ...
        for x in s.get("xrefs", []):
            ...
            dst = (
                None
                if str(x.get("peer", "")).startswith("EXT:")
                else _sheet_of_peer(str(x.get("peer", "")))
            )
            slot = terminals.setdefault(_norm(term), {"raw": term, "peers": set(), "sheets": set()})
            slot["sheets"].add(sid)
            if dst:
                slot["peers"].add(dst)
    ...
    for s in sheets:
        ...
        for c in s.get("contact_chains", []):
            slot = chains.setdefault(str(c.get("chain")), {"forms": set(), "sheets": set()})
    ...
    for chain, slot in chains.items():
        ...
        if len(forms) > 1:
            ...
            contradictions.append(
                {
                    "type": "contact_semantic_conflict",
                    "chain": chain,
                    "forms": sorted(forms),
                    "sheets": sorted(slot["sheets"]),
                    "safety": True,
    ...
    outs = {
        (_norm(e["sig"]), e["src"], e["dst"]) for e in edges if e.get("dir") == "out" and e["dst"]
    }
    ...
    for sig, src, dst in outs:
        if (sig, dst, src) in outs:
            ...
            if pair in seen_pairs:
                continue
            seen_pairs.add(pair)
            raw_sig = next(e["sig"] for e in edges if _norm(e["sig"]) == sig and e["src"] in pair)
            # a phantom always-driven signal is inherently safety-relevant
            contradictions.append(
                {
                    "type": "impossible_continuity",
                    "sig": raw_sig,
                    "sheets": sorted((src, dst)),
                    "safety": True,
                }
            )
    ...
    return {
        ...
        "summary": {
            "sheets": len(sheets),
            "observable_sheets": sum(1 for q in quality.values() if q in QUALITY_OBSERVABLE),
