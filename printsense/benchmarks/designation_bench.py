...
def run_benchmark() -> dict:
    ...
    connection_point_accuracy = cp_pass / cp_possible if cp_possible > 0 else 1.0
    ...
    for fix, d in decoded:
        ...
        if role and conv.get("state_proof") == "never":
            role_pass += 1
    role_possible = sum(1 for fix, d in decoded if d.get("connection_point") is not None)
    terminal_role_accuracy = role_pass / role_possible if role_possible > 0 else 1.0
    ...
    if digit_samples:
        ...
        if p["selected_profile"] != "eplan_iec":
            profile_pass += 1
    profile_selection_accuracy = profile_pass / 2 if aspect_samples and digit_samples else 1.0
    ...
    for fix in ambig_fixtures:
        try:
            ...
            pass
    ambiguity_calibration = ambig_pass / len(ambig_fixtures) if ambig_fixtures else 1.0
            ...
            for rel in rels:
                if rel["type"] in ("CONFIRMED_ALIAS_OF", "PROBABLE_ALIAS_OF"):
                    ...
                    hard_failures.append(
                        f"false alias: relate({raw1!r}, {raw2!r}) emitted {rel['type']}"
                    )
        except Exception:
            pass
    # a RATE: 0.0 is clean, 1.0 is every known-not-alias pair falsely aliased
    false_alias_rate = false_alias_count / len(non_alias_pairs) if non_alias_pairs else 0.0

    # Metric 8: false_continuity_rate
    # Check no decoded output asserts connection between sibling terminals
    continuity_pattern = re.compile(r"continuity|connected_to|bridged", re.IGNORECASE)
    continuity_count = 0
    for fix, d in decoded:
    ...
    false_continuity_rate = continuity_count / len(decoded) if decoded else 0.0
    ...
    raw_preservation = raw_preserve_count / len(decoded) if decoded else 0.0

    # Metric 10: safety_state_fabrication_rate
    # Check no state assertion outside safety caveat
    state_pattern = re.compile(
        r"is\s+closed|is\s+open|energi[sz]ed|safe\s+state|proves", re.IGNORECASE
    )
    safety_fail_count = 0
    for fix, d in decoded:
    ...
    safety_state_fabrication_rate = safety_fail_count / len(decoded) if decoded else 0.0
    ...
    for fix, d in decoded:
        ...
        if cp:
            ...
            if conv.get("state_proof") != "never":
                hard_failures.append(
                    f"hard-fail: {fix['raw']!r} connection_point lacks state_proof='never'"
                )
        ...
        if "~X4" in fix["raw"]:
            ...
            if "~" not in normalized:
                hard_failures.append(f"hard-fail: {fix['raw']!r} lost '~' in round-trip")
        ...
        if d.get("raw") != fix["raw"]:
            hard_failures.append(
                f"hard-fail: {fix['raw']!r} raw not preserved (got {d.get('raw')!r})"
            )
                ...
                try:
                    ...
                    for rel in rels:
                        if rel["type"] not in _relmod.RELATIONSHIP_TYPES:
                            hard_failures.append(
                                f"hard-fail: invalid relationship type {rel['type']!r}"
                            )
                except Exception:
                    pass
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--json", type=str, help="Output JSON result to file")
