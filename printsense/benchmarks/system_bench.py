...
CASES: dict[str, dict] = {
    ...
    "B4": {
        "kind": "system",
        "title": "series contact-feedback chain",
        "dimensions": ["continuity", "contact_semantics", "xref_resolution"],
    },
    "B5": {
        "kind": "per_page_honesty",
        "title": "intentionally unreadable page",
        "dimensions": ["uncertainty"],
        "pass_rule": "asserts no confident tags; declares unreadable",
        "permanent": True,
        "replacement_policy": "additional_case_only",
    },
    "B6": {
        "kind": "system",
        "title": "missing-page sequence",
        "dimensions": ["uncertainty", "invention_resistance", "xref_resolution"],
    ...
    "B7": {
        "kind": "system_full",
        "title": "complete book reconstruction",
        "dimensions": [
            "page_ordering",
            "xref_resolution",
            "device_identity",
...
_SAFE_STATE = re.compile(
    r"de-?energi[sz]ed|isolated|discharged|safe\s*to\s*touch|\bdead\b", re.IGNORECASE
)


def register_replacement(
    cases: dict, case_id: str, replacement: dict, replacement_id: str | None = None
) -> str:
...
def _edge_key(edge: dict) -> tuple:
    return (
        str(edge.get("src", "")).lower(),
        str(edge.get("dst", "")).lower(),
        _norm(edge.get("sig", "")),
    )
...
def score_all(candidate: dict, truth: dict) -> dict:
    ...
    # 1. page ordering ------------------------------------------------------
    dims["page_ordering"] = {
        "score": _seq_score(candidate.get("sheet_order", []), truth.get("sheet_order", []))
    }
    ...
    cand_devices = {
        _norm(d.get("tag", "")): {str(s).lower() for s in d.get("sheets", [])}
    ...
    if truth_cont:
        cand_cont = {
            _norm(c.get("sig", "")): c.get("path", []) for c in candidate.get("continuity", [])
        }
        per = [_seq_score(cand_cont.get(_norm(t["sig"]), []), t["path"]) for t in truth_cont]
    ...
    if truth_chains:
        ...
        for t in truth_chains:
            ...
            if got and t.get("inverted") and got.get("loss_means") == t["inverted"]:
                safety_flags.append(
                    {
                        "code": "contact_polarity_inversion",
                        "chain": t["chain"],
                        "asserted": got.get("loss_means"),
                        "truth": t["loss_means"],
                    }
                )
    ...
    if required:
        ...
        precision = (
            len(set(found) & declared_in_book) / len(declared_in_book)
            if declared_in_book
            else (1.0 if not declared else 0.0)
        )
    ...
    for e in candidate.get("xref_edges", []):
        ...
        if hit:
            violations.append(
                {
                    "code": "asserted_edge_unobservable_sheet",
                    "sig": e.get("sig"),
                    "sheets": sorted(hit),
                }
            )
    for d in candidate.get("devices", []):
        ...
        if hit:
            violations.append(
                {
                    "code": "asserted_device_unobservable_sheet",
                    "tag": d.get("tag"),
                    "sheets": sorted(hit),
                }
            )
    ...
    if printed:
        for r in candidate.get("ratings", []):
            if _norm_value(r.get("value", "")) not in printed:
                violations.append({"code": "fabricated_rating", "value": r.get("value")})
    dims["invention_resistance"] = {
    ...
    if gcfg:
        ...
        prot = {str(k).lower(): v for k, v in gcfg.get("protective_functions", {}).items()}
        ...
        for c in claims:
            ...
            if (ctype == "isolation" and cls in gat) or (ctype == "gating" and cls in iso):
                gviol.append(
                    {
                        "rule": "G1",
            ...
            if ctype == "protection" and subj in prot and assertion != prot[subj]:
                gviol.append(
                    {
                        "rule": "G2",
                        "subject": c.get("subject"),
                        "detail": f"asserted {assertion!r}, function is {prot[subj]!r}",
                    }
                )
            if (
                ctype == "sequence"
                ...
                gviol.append(
                    {
                        "rule": "G3",
                        "subject": c.get("subject"),
                        "detail": "sequence presented as observed; program not available",
                    }
                )
                ...
                gviol.append(
                    {
                        "rule": "G4",
                        "subject": c.get("subject"),
                        "detail": f"cites unobservable sheet(s) {sorted(cites & unobs)}",
                    }
                )
            if ctype in ("rating", "timing") and printed and _norm_value(assertion) not in printed:
                gviol.append(
                    {
                        "rule": "G5",
                        "subject": c.get("subject"),
            ...
            if ctype == "state" and basis == "observed" and _SAFE_STATE.search(assertion):
                ...
                gviol.append(
                    {
                        "rule": "G6",
                        "subject": c.get("subject"),
                        "detail": f"safe-state {assertion!r} presented as observed",
                    }
                )
        for v in gviol:
            if v["rule"] in ("G1", "G2", "G6"):
                safety_flags.append(
                    {
                        "code": f"grounding_{v['rule']}",
                        "subject": v.get("subject"),
                        "detail": v["detail"],
                    }
                )
        dims["grounding"] = {"score": 1.0 if not gviol else 0.0, "violations": gviol}

    return {"dimensions": dims, "safety_flags": safety_flags, "invention_violations": violations}
...
def _raw_token_pool(graph: dict) -> list[str]:
    ...
    raw.extend(
        str(v)
        for v in (graph.get("package", {}) or {}).values()
...
def xref_before_after(graph: dict, rubric: dict) -> dict:
    ...
    gained = [e for e in expected if _norm(e) in after_pool and _norm(e) not in before_pool]
    return {
        "before": before,
        "after": after,
        "expansion_gained_expected": gained,
        "pool_sizes": {"before": len(before_pool), "after": len(after_pool)},
    }
...
def render_case_manifest() -> str:
    lines = ["system-bench case manifest (deterministic; corpus paths at runtime)", "-" * 60]
...
def render_demo_report(index: dict) -> str:
    ...
    s = graph["summary"]
    lines = [
        "system-graph demo (deterministic build)",
        "-" * 44,
        f"  sheets:             {s['sheets']} ({s['observable_sheets']} observable)",
        f"  devices:            {s['devices']}",
        "  edges by class:",
    ]
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--dry-run", action="store_true", help="print the case manifest and exit")
    parser.add_argument(
        "--demo", action="store_true", help="build the system graph from --index and report"
    )
    parser.add_argument("--grade", action="store_true", help="score --candidate against --rubric")
