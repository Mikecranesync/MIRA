...
def classify_scope(graph: dict, index: dict, scope: dict) -> dict:
    ...
    for edge in graph.get("edges", []):
        ...
        if cls == "external":
            ...
            scope_state_counts["not_applicable"] = scope_state_counts.get("not_applicable", 0) + 1
            continue
...
def _assign_scope_state(edge: dict, index: dict, scope: dict) -> str:
    ...
    if cls == "dangling" and dst is not None:
        ...
        inventory_hit = bool(sheet_inventory) and (
            ...
            or str(dst).lstrip("Ss") in {str(s).lstrip("Ss") for s in sheet_inventory}
        )
        range_hit = False
        ...
        if scope_status == "partial_declared":
            ...
            return "missing_expected_page" if inventory_hit or range_hit else "invalid_reference"
