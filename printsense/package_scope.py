...
def classify_scope(graph: dict, index: dict, scope: dict) -> dict:
    ...
    for edge in graph.get("edges", []):
        ...
        # External edges are excluded from references but counted in original_edge_classes
        if cls == "external":
            continue
...
def _assign_scope_state(edge: dict, index: dict, scope: dict) -> str:
    ...
    if cls == "dangling" and dst is not None:
        ...
        # Check if dst is in scope
        in_scope = False
        if sheet_inventory and dst in sheet_inventory:
            in_scope = True
        elif declared_range and len(declared_range) >= 2:
            lo_str, hi_str = declared_range[0], declared_range[1]
            # Numeric prefix compare (S10, S15, S20, etc.)
            try:
                dst_num = int(re.match(r"S(\d+)", dst).group(1)) if re.match(r"S(\d+)", dst) else 0
                lo_num = int(re.match(r"S(\d+)", lo_str).group(1)) if re.match(r"S(\d+)", lo_str) else 0
                hi_num = int(re.match(r"S(\d+)", hi_str).group(1)) if re.match(r"S(\d+)", hi_str) else 0
                in_scope = lo_num <= dst_num <= hi_num
            except (ValueError, AttributeError):
                in_scope = False

        if scope_status == "partial_declared":
            return "missing_expected_page" if in_scope else "out_of_scope"
        elif scope_status == "complete_declared":
            return "missing_expected_page" if in_scope else "invalid_reference"
