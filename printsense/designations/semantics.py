...
_COLOR_CORES = {"GNYE", "GN", "YE", "BU", "BN", "BK", "GY", "WH", "RD", "OG", "VT", "PK", "TQ"}
...
def classify_connection_point(cp: str, parent_class: str | None) -> dict:
    ...
    if conv is not None and parent_class not in {"X", "XS", "W"}:
        if (
            conv.get("role") == "coil_or_control_terminal"
            and parent_class not in _COIL_BEARING_CLASSES
        ):
            return {
                "raw": cp,
                "kind": "connection_point",
                "convention": {
                    "role": "a_series_terminal_label",
        ...
        return out
    if parent_class == "W" and (cp.upper() in _COLOR_CORES or _NUMERIC.match(cp)):
        return {
            "raw": cp,
            "kind": "conductor_core",
            "convention": {
                "role": "core_color_code" if not _NUMERIC.match(cp) else "numbered_core",
                "state_proof": "never",
            },
        }
    if parent_class == "XS":
    ...
    if _ALNUM_POINT.match(cp) and not _NUMERIC.match(cp):
        return {
            "raw": cp,
            "kind": "port",
            "convention": {"role": "port_or_interface", "state_proof": "never"},
        }
    return {
        "raw": cp,
        "kind": "connection_point",
