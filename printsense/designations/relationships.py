...
def relate(d1: dict, d2: dict) -> list[dict]:
    ...
    if base1 and base1 == base2:
        rels.append({"type": "SAME_BASE_DEVICE", "from": a, "to": b, "base": base1})
        cp1, cp2 = d1.get("connection_point"), d2.get("connection_point")
        if cp1 and cp2 and cp1.get("pair_key") and cp1.get("pair_key") == cp2.get("pair_key"):
            rels.append(
                {
                    "type": "CONTACT_MEMBER_OF",
                    "from": a,
                    "to": b,
...
def migrate_alias_variations(graph: dict, profile: str = "eplan_iec") -> dict:
    ...
    findings = [c for c in graph.get("contradictions", []) if c.get("type") == "alias_variation"]
    reinterpretations: list[dict] = []
    counts = {
        "same_exact_designation": 0,
        "same_base_device": 0,
        "child_connection_point": 0,
        "probable_alias": 0,
        "unresolved": 0,
        "contradiction": 0,
    }
    ...
    for finding in findings:
        ...
        anchor = max(base_counts, key=lambda b: (base_counts[b], -len(b))) if base_counts else None
        members: list[dict] = []
        for form in forms:
            d = decoded[form]
