...
def get_profile(name: str | None) -> dict:
    ...
    if not prof.get("proven"):
        prof = {
            **PROFILES["unknown_european"],
            "name": name,
            "diagnostic": f"profile {name!r} declared but not proven; conservative rules applied",
        }
    return prof
...
def detect_profile(samples: list[str], title_text: str = "") -> dict:
    ...
    alternatives = (
        [{"name": "unknown_european", "confidence": 0.4}]
        if selected == "eplan_iec"
        else [{"name": "eplan_iec", "confidence": round(min(score, 0.45), 2)}]
    )
    return {
