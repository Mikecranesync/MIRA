...
def decode(
    ...
    if prof.get("diagnostic"):
        diagnostics.append({"code": "profile_fallback", "detail": prof["diagnostic"]})
    for stage in (
        "package_context",
        "project_profile_resolution",
    ...
    for tok in lexed["tokens"]:
        for cand in tok.get("ocr_candidates", []):
            ambiguities.append(
                {"raw": tok["raw"], "candidates": [cand], "reason": "possible OCR substitution"}
            )
    ...
    for seg in parsed["segments"]:
        ...
        if seg.get("kind") == "device_candidate":
            ...
            ambiguities.extend(
                legend_conflicts(legends, seg["class_code"], info["candidate_classes"])
            )
        segments.append(seg)
    ...
    if parsed["connection_point_raw"] is not None:
        ...
        if device_profile:
            note = (device_profile.get("terminals") or {}).get(connection_point["raw"])
            if note:
    ...
    if connection_point:
        ...
        relationship = _child_relationship({"connection_point": connection_point}) or None
    ...
    decoded = {
        ...
        "entity_plan": {"parent_device": base, "child_entity": child, "relationship": relationship},
...
def explain(decoded: dict, max_items: int | None = None) -> str:
    ...
    if decoded.get("base_designation"):
        body.append(
            f"- {decoded['base_designation']} identifies the parent "
    ...
    if cp:
        body.append(
            f"- {cp['raw']}: {cp.get('role', cp['kind'])} on that "
