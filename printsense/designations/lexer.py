...
_OCR_WHOLE = {
    "Al": ("A1", "l->1 after letter"),
    "AZ": ("A2", "Z->2 after letter"),
...
def _ocr_candidates(text: str) -> list[dict]:
    ...
    if text in _OCR_WHOLE:
        ...
        out.append({"kind": "ocr_correction", "text": fixed, "reason": reason, "confidence": 0.6})
...
def lex(raw: str) -> dict:
    ...
    def emit(kind: str, start: int, end: int) -> None:
        text = raw[start:end]
        tok = {"raw": text, "text": text, "kind": kind, "start": start, "end": end}
        if kind == "text":
    ...
    while i < n:
        ...
        if m:
            ...
            while (
                end > i + 1 and raw[end - 1] in _SOFT_DOT and not (end < n and raw[end].isalnum())
            ):
                end -= 1
            emit("text", i, end)
            i = end
