...
_SRC_60947 = {
    "source_title": "Low-voltage switchgear - control circuit "
    "devices: terminal marking scheme (derived)",
    "organization": "IEC/CENELEC",
    "document_id": "IEC 60947-5-1",
...
_SRC_50005 = {
    "source_title": "Terminal markings for control devices incl. coil terminals (derived)",
    "organization": "CENELEC",
    "document_id": "EN 50005/50011",
    "section": "coil terminal designations (derived)",
    "confidence": 0.7,
    "reviewed_date": "2026-07-15",
    "verification": "secondary_sources_only",
    "note": "A1/A2 designate coil terminals; polarity is NOT derivable from the marking alone",
}
_SRC_MAIN = {
    "source_title": "Main-circuit terminal markings for contactors and overload relays (derived)",
    "organization": "IEC/CENELEC",
    "document_id": "IEC 60947-4-1",
...
def classify(cp: str, parent_class: str | None = None) -> dict | None:
    ...
    if _COIL.match(cp):
        return {
            ...
            "convention": {
                "role": "coil_or_control_by_convention",
                "polarity": "unknown",
                "state_proof": "never",
                "source": _SRC_50005,
            },
    ...
    if m:
        ...
        if cp in ("95", "96", "97", "98"):
            ...
            return {
                "kind": "contact_terminal",
                "pair_key": "95-96" if cp in ("95", "96") else "97-98",
                "convention": {
                    "sequence": seq,
                    "function_digit": func,
                    "role": (
                        "overload_NC_by_convention"
                        if cp in ("95", "96")
    ...
    if _MAIN_POLE.match(cp):
        return {
            "kind": "main_pole_terminal",
            "convention": {
                "role": "main_pole_by_convention",
                "printed": cp,
                "state_proof": "never",
                "source": _SRC_MAIN,
            },
