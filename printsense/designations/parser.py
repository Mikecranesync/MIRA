...
def parse(lexed: dict, profile: dict, page_context: dict | None = None) -> dict:
    ...
    colon_ix = next(
        (i for i, t in enumerate(tokens) if t["kind"] == "sep" and t["raw"] == ":"), None
    )
    ...
    if colon_ix is not None and profile.get("colon_is_connection_point", True):
        ...
        if len(right_text) == 1 and not any(t["kind"] == "sep" and t["raw"].strip() for t in right):
    ...
    for tok in left:
        ...
        if aspect == "function" and out["aspects"]["function"] is None:
            ...
            if m:
                ...
                seg["raw"] = ("-" if aspect == "product" and f"-{text}" in raw else "") + text
        ...
        if seg["kind"] in ("unresolved", "product_or_structure_candidate"):
            out["unresolved_segments"].append(
                {
                    "raw": seg["raw"],
                    "reason": "meaning requires structure profile or project legend",
                }
            )
    ...
    if prefix and not any(a in raw for a in "=+"):
        ...
        if stripped:
            out["resolved_full_designation"] = f"{prefix}-{stripped}" + (
                f":{out['connection_point_raw']}" if out["connection_point_raw"] else ""
            )

    if not any(s["kind"] not in ("unresolved",) for s in out["segments"]) and not out["segments"]:
        out["unresolved_segments"].append({"raw": raw, "reason": "no parseable content"})
        out["diagnostics"].append(
            {"code": "malformed_designation", "detail": "no text tokens found"}
        )
    if out["segments"] and all(s["kind"] == "unresolved" for s in out["segments"]):
        out["diagnostics"].append({"code": "unparsed_designation", "detail": "no segment resolved"})
    return out
