...
def build_identity_graph(index: dict, legends: list | None = None) -> dict:
    ...
    for sheet_entry in index.get("sheets", []):
        ...
        for dev in sheet_entry.get("devices", []):
            ...
            prov = {**(dev.get("provenance") or {}), "confidence": dev.get("confidence")}
            ...
            if cp is None:
                ...
                _rel(rels, "CHILD_CONNECTION_POINT_OF", dev["tag"], base, "token", evidence, 0.85)
                if primary_rel != "CHILD_CONNECTION_POINT_OF":
                    _rel(rels, primary_rel, dev["tag"], base, "token", evidence, 0.85)
                pair = cp.get("pair_key")
                if pair:
                    group_key = f"{base}:{pair}"
                    group = _ensure_node(nodes, group_key, "ContactGroup", page, sheet, None)
            ...
            if prefix and resolved and resolved != dev["tag"]:
                ...
                _rel(
                    rels,
                    "RELATIVE_REFERENCE_TO",
                    dev["tag"],
                    resolved,
                    "page_context",
                    {"prefix": prefix, "page_id": page},
                    0.7,
                )
        ...
        for u in sheet_entry.get("unresolved", []) or []:
            entry = {"raw": u.get("item"), "reason": u.get("status"), "page_id": page}
            if entry not in unresolved:
                unresolved.append(entry)
    ...
    for key, node in nodes.items():
        if (
            node["entity_type"] in ("Device", "Subdevice")
...
def query_designation(graph: dict, raw: str, systemgraph_result: dict | None = None) -> dict:
    ...
    parent_key = next(
        (r["to"] for r in rels if r["from"] == raw and r["type"] in _PARENT_RELS), None
    )
    children = [
        nodes[r["from"]]
        for r in rels
        if r["to"] == raw and r["type"] in _PARENT_RELS and r["from"] in nodes
    ]
    seen: list = []
    children = [c for c in children if c["key"] not in seen and not seen.append(c["key"])]
    groups = [
        n
    ...
    return {
        ...
        "pages": sorted(
            {p for c in ([node] if node else []) + children for p in c.get("pages", [])}
        ),
        "relationships": [r for r in rels if raw in (r["from"], r["to"])],
        "ambiguities": [
            a
            for a in graph.get("ambiguities", [])
            if raw in json.dumps(a) or a.get("kind") == "revision_conflict"
        ],
        "unresolved": [u for u in graph.get("unresolved", []) if u.get("designation") == raw],
        "contradictions": contradictions,
    }


def reinterpret_with_records(
    legacy_graph: dict, index: dict | None = None, profile: str = "eplan_iec"
) -> dict:
    ...
    for sheet_entry in (index or {}).get("sheets", []):
        for dev in sheet_entry.get("devices", []):
            ...
            prov.setdefault(dev["tag"], (p.get("page_id"), p.get("section")))
            ...
            members.append(
                {
                    ...
                    "confidence": 0.9 if m["relationship"] != "AMBIGUOUS_WITH" else 0.4,
                    "ambiguities": []
                    if m["relationship"] != "AMBIGUOUS_WITH"
