...
_SYSTEM = (
    ...
    "- HOMOGLYPHS — letters and digits are NOT interchangeable. `I`/`1`, `O`/`0`, "
    ...
    "letters: `P9DI900-0` is not `P9D900-0`, and `P9-DI9-00` is not `P9-D9-00`. "
    "Losing one character makes the designation unsearchable for a technician.\n"
    ...
    "TAG GRAMMAR — the DRAWING is the authority; a convention is only a PRIOR.\n"
    "First decide which convention this drawing follows (title block, tag shapes, "
    "wire-number style), then read it with that convention's grammar below. A "
    "convention tells you what is LIKELY, never what is true: if a legible "
    "designation does not fit the expected grammar, the designation wins — "
    "transcribe it verbatim and note the mismatch in `evidence`. NEVER rewrite a "
    "character you can see in order to satisfy a standard.\n\n"
    "(a) DIN/IEC 81346 — European drawings:\n"
    ...
    "(b) NORTH AMERICAN / Rockwell — US drawings (JIC / NFPA 79 style). These "
    "obey a DIFFERENT grammar; do NOT force them into the IEC forms above:\n"
    "- Wire number: an alphanumeric compound, usually with NO leading `-`. It "
    "mixes letters, digits, `-`, `+`, `/` and `.` freely (e.g. `PW99+1A`, "
    "`TB99COM3`, `CB99UPS-1`, `P9DI900-0`, `TBX9-2A`), and may be plain digits "
    "(`0901`, `0902`). A US wire number is NOT `-W####` — never add a `-W` "
    "prefix, and never strip a letter run to make one.\n"
    "- PLC address: Allen-Bradley/SLC forms `I:1/0`, `O:2/7`, `B3:0/1`, `N7:20`, "
    "`T4:1.DN`, and ControlLogix forms `Local:1:I.Data[0]`. The `:`, `/`, `.` and "
    "`[]` are PART of the address — copy them exactly.\n"
    "- Device tag: a function abbreviation + number (e.g. `MTR-01`, `CR1-13`, "
    "`PB1-START`, `LS-12`, `SOL-3`, `CB-PW99`).\n"
    "- Wire COLOUR/GAUGE annotations (`BLU #18`, `WHT/BLU #18`, `GRN/YEL #10`, "
    "`BLK #8`) DESCRIBE a conductor — they are not conductors themselves and are "
    "not wire numbers. Record them in that conductor's `detail`; never emit one "
    "as a conductor `tag`.\n\n"
