...
# PII patterns — kept in sync with mira-bots/shared/inference/router.py
_IPV4_RE = re.compile(
...
def _sanitize_messages(messages: list[dict]) -> list[dict]:
    ...
    for msg in messages:
        ...
        if isinstance(content, str):
            sanitized.append({**msg, "content": _sanitize_text(content)})
        elif isinstance(content, list):
            new_blocks = []
            for block in content:
                if isinstance(block, dict) and block.get("type") == "text":
                    new_blocks.append({**block, "text": _sanitize_text(block.get("text", ""))})
                else:
