...
def build_case_sql(rules: list[tuple[str, list[str]]], tenant_id: str) -> str:
    ...
    searchable = (
        "LOWER(content || ' ' || COALESCE(source_url, '') || ' ' || COALESCE(manufacturer, ''))"
    )
    when_clauses = []
...
def run(dry_run: bool, batch_size: int) -> None:
    ...
    with engine.connect() as conn:
        ...
        before = conn.execute(
            text(
                "SELECT COUNT(*) FROM knowledge_entries WHERE tenant_id = :tid AND equipment_type IS NULL"
            ),
            {"tid": tenant_id},
        ).scalar()
        ...
        rows = conn.execute(
            text(
                "SELECT equipment_type, COUNT(*) as n FROM knowledge_entries "
