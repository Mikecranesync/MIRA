...
DANFOSS_VLT: list[tuple[str, str, str, str, str]] = [
    ("2", "Live Zero Error", "Control signal on analog input below 50% of minimum live zero value.",
     "1. Check analog input wiring. 2. Verify signal source is within range. 3. Check parameter setup for analog input scaling.",
     "trip"),
    ("4", "Mains Phase Loss", "A phase is missing on the mains supply side, or the mains voltage imbalance is too high.",
     "1. Check supply voltage on all three phases. 2. Verify fuses. 3. Check input contactor.",
     "trip"),
    ("5", "DC Link High Voltage", "DC link voltage exceeds the overvoltage limit.",
     ...
     "trip"),
    ("7", "DC Link Overvoltage", "DC link voltage exceeds the inverter's absolute maximum.",
     "1. Increase deceleration time. 2. Add braking resistor if regenerative load. 3. Check supply voltage for spikes.",
     "trip"),
    ("8", "DC Link Undervoltage", "DC link voltage dropped below the undervoltage limit.",
     "1. Check mains supply voltage. 2. Check for voltage sags. 3. Verify power supply capacity.",
     "trip"),
    ("9", "Inverter Overload", "Inverter thermal protection activated — too much current for too long.",
     "1. Reduce load or duty cycle. 2. Check for mechanical binding. 3. Verify motor sizing matches application.",
     "trip"),
    ("10", "Motor Overtemperature ETR", "Electronic thermal relay estimates motor is overheated.",
     "1. Check if motor is overloaded. 2. Verify motor cooling fan operates. 3. Check ETR parameters match motor thermal capacity.",
     "trip"),
    ("11", "Motor Thermistor Overtemp", "Motor thermistor or KTY sensor has disconnected or signals overtemperature.",
     "1. Check thermistor wiring. 2. Measure thermistor resistance. 3. Allow motor to cool down.",
     "trip"),
    ("13", "Overcurrent", "Inverter peak current limit exceeded (approx. 200% of rated).",
...
YASKAWA_A1000: list[tuple[str, str, str, str, str]] = [
    ("OC", "Overcurrent", "Output current exceeded 200% of drive rated current during acceleration, deceleration, or at constant speed.",
     "1. Extend accel/decel times. 2. Check for motor short circuit. 3. Reduce load. 4. Verify motor parameters.",
     "trip"),
    ("GF", "Ground Fault", "Ground fault current exceeded 50% of drive rated current.",
     "1. Megger motor insulation. 2. Check motor cable insulation. 3. Check for moisture in motor or junction box.",
     "trip"),
    ("OV", "DC Bus Overvoltage", "DC bus voltage exceeded the overvoltage level during operation.",
     "1. Extend deceleration time. 2. Enable overvoltage suppression. 3. Add braking resistor for regenerative loads.",
     "trip"),
    ("UV1", "DC Bus Undervoltage", "Main circuit DC bus voltage dropped below undervoltage level during operation.",
     "1. Check input power supply. 2. Check for voltage sags. 3. Verify supply capacity for load.",
     "trip"),
    ("UV2", "DC Bus Undervoltage at Powerup", "Control circuit power supply voltage too low at startup.",
     "1. Check main circuit power. 2. Verify supply voltage matches drive rating.",
     "trip"),
    ("UV3", "Undervoltage Transient", "Momentary power loss detected — DC bus dropped briefly.",
     "1. Check input power quality. 2. Enable momentary power loss ride-through (L2-01). 3. Check for upstream switching.",
     "trip"),
    ("LF", "Output Phase Loss", "One of three output phases is not connected or is open.",
...
ABB_ACS580: list[tuple[str, str, str, str, str]] = [
    ("1", "Overcurrent", "Output current exceeded trip limit.",
...
GS_SUPPLEMENT: list[tuple[str, str, str, str, str, str, str]] = [
    ...
    ("OC", "Overcurrent at Constant Speed", "Output current exceeded 200% at constant speed.",
     "1. Check for sudden load change. 2. Check motor insulation. 3. Reduce load.",
     "trip", "AutomationDirect", "GS10"),
    ("OCA", "Overcurrent During Accel", "Output current exceeded 200% during acceleration.",
     "1. Extend accel time. 2. Check for mechanical binding. 3. Reduce load.",
     "trip", "AutomationDirect", "GS10"),
    ("OCD", "Overcurrent During Decel", "Output current exceeded 200% during deceleration.",
     "1. Extend decel time. 2. Add braking resistor. 3. Use coast-to-stop.",
     "trip", "AutomationDirect", "GS10"),
    ("OV", "Overvoltage", "DC bus voltage exceeded trip level.",
    ...
    ("GF", "Ground Fault", "Ground fault detected at output.",
     "1. Megger motor. 2. Check cable insulation. 3. Check for moisture.",
     "trip", "AutomationDirect", "GS20"),
    ("PHL", "Input Phase Loss", "Missing input phase detected.",
     "1. Check all three input phases. 2. Check fuses. 3. Check contactor.",
     "trip", "AutomationDirect", "GS20"),
    ("OPL", "Output Phase Loss", "Missing output phase to motor.",
    ...
    ("EF", "External Fault", "External fault signal received on input terminal.",
     "1. Check external fault source. 2. Check input wiring. 3. Clear upstream condition.",
     "trip", "AutomationDirect", "GS20"),
    ("STP", "Motor Stall Prevention", "Motor stall detected during operation.",
     "1. Check for mechanical jam. 2. Reduce load. 3. Verify motor sizing.",
     "warning", "AutomationDirect", "GS20"),
    ("AUF", "Auto-tuning Fault", "Motor auto-tuning failed.",
...
POWERFLEX_SUPPLEMENT: list[tuple[str, str, str, str, str, str, str]] = [
    ...
    ("F5", "OverVoltage", "DC bus voltage exceeded maximum limit.",
     ...
     "trip", "Rockwell Automation", "PowerFlex 40"),
    ("F7", "Motor Overload", "Electronic thermal overload relay tripped.",
     "1. Reduce load. 2. Verify motor FLA parameter. 3. Check motor cooling.",
     "trip", "Rockwell Automation", "PowerFlex 40"),
    ("F8", "Drive Overload", "Drive output current exceeded rating for too long.",
     "1. Reduce load or cycle time. 2. Extend accel time. 3. Use larger drive.",
     "trip", "Rockwell Automation", "PowerFlex 40"),
    ("F12", "HW OverCurrent", "Hardware overcurrent — IGBT overcurrent on output.",
    ...
    ("F13", "Ground Fault", "Ground fault detected on drive output.",
     ...
     "trip", "Rockwell Automation", "PowerFlex 40"),
    ("F29", "Analog Input Loss", "4-20mA signal dropped below 4mA.",
     "1. Check transmitter. 2. Check wiring. 3. Verify loop power supply.",
     "trip", "Rockwell Automation", "PowerFlex 40"),
    ("F41", "Heatsink OvrTmp", "Heatsink overtemperature.",
     "1. Check ambient temp. 2. Clean heatsink. 3. Check fan. 4. Reduce load.",
     "trip", "Rockwell Automation", "PowerFlex 40"),
    ("F48", "Phase Loss", "Input or output phase loss.",
     "1. Check input phases. 2. Check output cable connections. 3. Check fuses.",
     "trip", "Rockwell Automation", "PowerFlex 40"),
    ("F64", "Software Overcurrent", "Software current limit exceeded.",
     "1. Extend accel time. 2. Reduce load. 3. Check motor parameters.",
     "trip", "Rockwell Automation", "PowerFlex 40"),
]


def _insert(conn, text_fn, tenant_id: str, code: str, description: str,
            cause: str, action: str, severity: str,
...
def main() -> None:
    ...
    parser.add_argument("--dry-run", action="store_true",
                        help="Print codes without inserting")
