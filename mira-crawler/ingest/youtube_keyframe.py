...
def _classify_frame(frame_path: Path) -> dict | None:
    """Classify frame via Ollama qwen2.5vl:7b vision (local, no API key required)."""
    ollama_url = os.getenv("OLLAMA_BASE_URL", "http://localhost:11434")
    model = os.getenv("YOUTUBE_VISION_MODEL", "qwen2.5vl:7b")

    if frame_path.stat().st_size < 1000:
        logger.warning("Skipping tiny/corrupt frame %s (%d bytes)", frame_path.name, frame_path.stat().st_size)
        return None
