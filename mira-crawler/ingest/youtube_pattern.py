...
def _claude_extract_pattern(transcript: str) -> dict | None:
    """Call Claude to extract teaching pattern JSON from transcript text."""
    ...
    # Truncate transcript to ~12k chars to stay within context budget
    ...
    try:
        ...
        logger.warning("JSON parse error in pattern extraction for: %s", e)
        return None
    except Exception as e:
