...
def _ext(tag, roles=None, ev=None, uns=None, status="pending"):
    return {"tagName": tag, "roles": roles or [], "unsPathProposed": uns,
            "evidenceJson": ev or {}, "confidence": 0.9, "status": status}
...
def test_ccw_tags_without_units_or_fault_cause_flags_those_gaps():
    exts = [
        ...
        _ext("motor_running", ["motor", "status"],
             {"data_type": "Bool", "modbus_address": "000001", "comment": "run fb"},
             uns="enterprise/s/a/l/cv/run", status="accepted"),
        _ext("fault_alarm", ["fault_code"], {"data_type": "Bool", "modbus_address": "000003"},
             status="accepted"),
    ]
...
def test_units_and_fault_semantics_raise_score_and_clear_gaps():
    exts = [
        _ext("speed", ["analog"], {"units": "RPM", "range": "0-1800", "data_type": "Int",
                                   "modbus_address": "400101", "comment": "drive speed"},
             uns="x", status="accepted"),
        _ext("F0004", ["fault_code"], {"cause": "overload", "next_check": "check amps",
                                       "comment": "overload"}, status="accepted"),
    ]
