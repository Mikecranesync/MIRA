...
def test_ccw_project_import_json_and_zip(base):
    ...
    st, j = _req(
        f"{base}/api/projects/{pid}/ccw-import",
        "POST",
        {
            "projectName": "FactoryLM_PLC",
            "files": [
                {"name": "MbSrvConf.xml", "text": MODBUS},
    ...
    req = urllib.request.Request(
        ...
        data=buf.getvalue(),
        method="POST",
        headers={"Content-Type": "application/octet-stream", "X-Project-Name": "backup.zip"},
    )
    with urllib.request.urlopen(req) as r:
...
def test_text_upload_falls_back_to_document_contextualization(base):
    ...
    st, j = _req(
        f"{base}/api/projects/{pid}/sources",
        "POST",
        {"fileName": "notes.xml", "text": "<x>drive fault F0004 overload trip</x>"},
    )
    assert st == 201 and j["extractions"] >= 1
    _, ext = _req(f"{base}/api/projects/{pid}/extractions")
...
def test_document_upload_extracts_text_and_contextualizes(base):
    ...
    _, ext = _req(f"{base}/api/projects/{pid}/extractions")
    assert any(e["tagName"] == "F0004" and "fault_code" in e["roles"] for e in ext["extractions"])
