...
def test_docx(tmp_path):
    ...
    d = docx.Document()
    d.add_paragraph("Drive trips on overload F0004.")
...
def test_pdf_digital_text(tmp_path):
    p = tmp_path / "d.pdf"
    p.write_bytes(_min_pdf("Fault F0004 overload"))
    r = extract.extract(str(p))
