...
GRADES = [
    ...
    (65, "Diagnosable", "Faults + descriptions present; add units/ranges + cause mapping to close the loop."),
...
def compute_scorecard(extractions: list[dict], sources: list[dict]) -> dict:
    ...
    has_docs = any((s.get("sourceType") == "manual") for s in sources) or \
        any(x.get("source") == "document" for x in ev)
    has_faults = any(("fault_code" in (e.get("roles") or []) or "fault" in (e.get("roles") or []))
    ...
    dims = [
        ("signals", "Signals captured", 1.0, 1.0 if n else 0.0,
         "Upload MbSrvConf.xml / LogicalValues.csv to capture the tag list.", 0),
        ("identity", "Controller identity", 1.0, 1.0 if has_controller else 0.0,
         "Add the .st/.stf source or DevicePref.xml so the controller model + IP are captured.", 1),
        ("types", "Data types", 1.0, _frac(with_type, n),
         "Include MbSrvConf.xml or .iecst declarations so each tag has a data type.", 1),
        ("addressing", "Modbus / I-O addressing", 1.0, _frac(with_addr, n),
         "Add MbSrvConf.xml / .ccwmod so signals carry Modbus addresses.", 1),
        ("descriptions", "Descriptions / comments", 1.5, _frac(with_desc, n),
         "Add the .st/.stf/.iecst sources (inline comments) — the .accdb symbol descriptions if exportable.", 1),
        ("roles", "Role classification", 1.0, _frac(with_role, n),
         "Review tags so motor/fault/sensor/safety roles are confirmed.", 1),
        ("io_mapping", "Physical I-O mapping", 1.0, 1.0 if has_terminal else 0.0,
         "Add ST comments mapping terminals to signals (I-02 = e_stop_active).", 1),
        ("documents", "Equipment docs", 1.5, 1.0 if has_docs else 0.0,
         "Upload the machine/drive manual (Documents tab) for cited answers.", 1),
        ("faults", "Fault catalog", 1.5, 1.0 if has_faults else 0.0,
         "Upload the manual so fault codes are extracted, or confirm fault-role tags.", 1),
        ("approved", "Human-approved", 1.0, _frac(accepted, len(extractions)),
         "Accept the correct proposals so they are trusted, not just suggested.", 1),
        ("units_ranges", "Units / ranges / setpoints", 2.0, 1.0 if has_units else 0.0,
         "Capture engineering units + normal ranges — without them no value can be judged abnormal.", 2),
        ("fault_semantics", "Fault cause -> next-check", 2.0, 1.0 if has_fault_semantics else 0.0,
         "Map each fault to a likely cause + the next thing to check (the core of a useful answer).", 2),
        ("placement", "UNS placement", 1.5, _frac(with_uns, n),
         "Assign each accepted signal a UNS path (site/area/line/asset/signal).", 2),
    ]
    ...
    dimensions = [{"key": k, "label": lbl, "weight": w, "coverage": round(cov, 2),
                   "tier": tier, "needed": None if cov >= 0.99 else need}
