...
from mira_plc_parser import cli, i3x
...
def test_cli_i3x_export_writes_payload_offline(tmp_path):
    rc = cli.main(["i3x-export", str(FIXTURE), "--out", str(tmp_path), "--quiet"])
    assert rc == 0
    out = tmp_path / "conveyor.i3x.json"
    assert out.is_file()
    payload = json.loads(out.read_text(encoding="utf-8"))
    assert payload["namespace"]["uri"] == i3x.NAMESPACE_URI
    assert any(i["metadata"].get("plcTag") == "VFD_Frequency" for i in payload["objectInstances"])
