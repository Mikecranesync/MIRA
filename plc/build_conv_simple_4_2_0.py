#!/usr/bin/env python3
"""Build the Conv_Simple_4.2.0 CCW project package.

This follows the older Claude/CCW package pattern:

1. Clone the existing v4-style CCW project (`MIRA_PLC.ccwsln` + `controller/`).
2. Rename the solution to `Conv_Simple_4.2.0.ccwsln`.
3. Bake `Micro820_v4.2.0_Program.st` into the clone's `Prog2.stf`.
4. Stage the install/variable/safety docs in `_V4.2.0_APPLY/`.
...
Usage on the PLC laptop with CCW closed:
    ...
    python plc/build_conv_simple_4_2_0.py --force
"""
...
PROGRAM_SRC = HERE / "Micro820_v4.2.0_Program.st"
STAGE_FILES = [
    PROGRAM_SRC,
    HERE / "CCW_VARIABLES_ConvSimple_v4.2.0_DELTA.md",
    HERE / "INSTALL_ConvSimple_v4.2.0.md",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build the Conv_Simple_4.2.0 CCW package.")
    ...
    print("Conv_Simple_4.2.0 package build plan")
    print("-" * 64)
    ...
    prog2 = dst / PROG2_REL
    backup = prog2.with_suffix(".stf.pre_v4_2_0.bak")
    ...
    print("\nThen follow:")
    print(f"  {apply_dir / 'INSTALL_ConvSimple_v4.2.0.md'}")
