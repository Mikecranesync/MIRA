# CCW Variables to Map - Conv_Simple v4.2.1 Discharge Acceptance

Bench/demo logic only. This is not production-certified machinery control.

`plc/Micro820_v4.2.1_Program.st` is the next ST program version derived from
`Micro820_v4.1.9_Program.st`. It adds SimLab discharge request handling with a
required local human acceptance step.

v4.2.1 fixes the v4.2.0 CCW build error by removing the undeclared `e_stop_ok`
