#!/usr/bin/env python3
"""Build the Conv_Simple_4.2.1 CCW project package.

This follows the older Claude/CCW package pattern:

1. Clone the existing v4-style CCW project (`MIRA_PLC.ccwsln` + `controller/`).
2. Rename the solution to `Conv_Simple_4.2.1.ccwsln`.
3. Bake `Micro820_v4.2.1_Program.st` into the clone's `Prog2.stf`.
4. Stage the install/variable/safety docs in `_V4.2.1_APPLY/`.
...
Usage on the PLC laptop with CCW closed:
    ...
    python plc/build_conv_simple_4_2_1.py --dry-run
    python plc/build_conv_simple_4_2_1.py
    python plc/build_conv_simple_4_2_1.py --force
"""
...
PROGRAM_SRC = HERE / "Micro820_v4.2.1_Program.st"
STAGE_FILES = [
    PROGRAM_SRC,
    HERE / "CCW_VARIABLES_ConvSimple_v4.2.1_DELTA.md",
    HERE / "INSTALL_ConvSimple_v4.2.1.md",
...
def check_baked_project(dst: Path) -> None:
    ...
    if "e_stop_ok :=" in text or "e_stop_ok" in text[text.index("NEW VARIABLES") : text.index("Physical I/O")]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build the Conv_Simple_4.2.1 CCW package.")
    ...
    print("Conv_Simple_4.2.1 package build plan")
    print("-" * 64)
    ...
    prog2 = dst / PROG2_REL
    backup = prog2.with_suffix(".stf.pre_v4_2_1.bak")
    shutil.copy2(prog2, backup)
    prog2.write_text(PROGRAM_SRC.read_text(encoding="utf-8"), encoding="utf-8")
    print(f"[2/4] baked {PROGRAM_SRC.name} -> {PROG2_REL} (backup {backup.name})")

    apply_dir = dst / "_V4.2.1_APPLY"
    apply_dir.mkdir()
    for file in STAGE_FILES:
    ...
    print("\nThen follow:")
    print(f"  {apply_dir / 'INSTALL_ConvSimple_v4.2.1.md'}")
