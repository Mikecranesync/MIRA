# INSTALL - Conv_Simple v4.2.1 Discharge Acceptance

Bench/demo only. Do not treat this as production-certified machinery control.

## What Changed

`plc/Micro820_v4.2.1_Program.st` adds supervised SimLab discharge request logic:
...
```cmd
plc\BUILD_CONV_SIMPLE_4.2.1.cmd
```

That creates:

```text
C:\Users\hharp\Documents\CCW\MIRA_PLC\Conv_Simple_4.2.1\Conv_Simple_4.2.1.ccwsln
```

The generated project is cloned from the v4-style `MIRA_PLC.ccwsln` controller
project and has `Micro820_v4.2.1_Program.st` already baked into `Prog2.stf`.
It does not edit CCW's binary symbol table or generated compile artifacts.

v4.2.1 fixes the CCW build failure from v4.2.0:

```text
PROG2(174,0): Error 0060-0000: E_STOP_OK: undeclared identifier
```

The fixed program no longer requires a new `e_stop_ok` global. It uses the
existing `e_stop_active` and `estop_wiring_fault` variables directly.

## Before Download

1. Read `plc/CCW_VARIABLES_ConvSimple_v4.2.1_DELTA.md`.
2. Declare the new BOOL and TON variables in CCW with blank Dimension fields.
3. Open `Conv_Simple_4.2.1\Conv_Simple_4.2.1.ccwsln`.
4. Confirm `DwlOrder.txt` / project organizer shows `Prog2` as the program POU.
5. Confirm no other routine writes `_IO_EM_DO_04`. The generated v4.2.1 package
