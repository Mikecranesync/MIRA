# INSTALL - Conv_Simple v4.2.2 Discharge Acceptance

Bench/demo only. Do not treat this as production-certified machinery control.

## What Changed

`plc/Micro820_v4.2.2_Program.st` adds supervised SimLab discharge request logic:
...
```cmd
plc\BUILD_CONV_SIMPLE_4.2.2.cmd
```

That creates:

```text
C:\Users\hharp\Documents\CCW\MIRA_PLC\Conv_Simple_4.2.2\Conv_Simple_4.2.2.ccwsln
```

The generated project is cloned from the v4-style `MIRA_PLC.ccwsln` controller
project and has `Micro820_v4.2.2_Program.st` already baked into `Prog2.stf`.
It does not edit CCW's binary symbol table or generated compile artifacts.

v4.2.2 fixes the CCW build failure from v4.2.0:
...
1. Read `plc/CCW_VARIABLES_ConvSimple_v4.2.2_DELTA.md`.
2. Declare the new BOOL and TON variables in CCW with blank Dimension fields.
3. Open `Conv_Simple_4.2.2\Conv_Simple_4.2.2.ccwsln`.
4. Confirm `DwlOrder.txt` / project organizer shows `Prog2` as the program POU.
5. Confirm no other routine writes `_IO_EM_DO_04`. The generated v4.2.2 package
