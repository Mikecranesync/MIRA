> **Note:** This is a sanitized version for public distribution. File paths and IP addresses have been generalized. `$REPO` = your MIRA repository root, `$CCW_PROJECT` = your CCW project directory.
...
PLC files verified at: $REPO/plc/
Git is at commit 83c053c, matches origin/main.
CCW project: $CCW_PROJECT/Cosmos_Demo_v1.0/
PLC laptop Ethernet IP: <PLC_LAN_IP>/24 (static)
PLC target IP: <PLC_LAN_IP>
```
...
Project repo is at: $REPO/
CCW project is at: $CCW_PROJECT/Cosmos_Demo_v1.0/
...
1a. ping -n 3 -w 1000 <PLC_LAN_IP>    <- PLC
1b. ping -n 3 -w 1000 <PLC_LAN_IP>      <- Switch/gateway
1c. Test-NetConnection -ComputerName <PLC_LAN_IP> -Port 502
...
If we cannot ping <PLC_LAN_IP>, we need to find the PLC another way.
...
Tell me to do this in CCW:
  ...
  - Controller Properties -> Ethernet -> set IP to <PLC_LAN_IP>
...
4a. Verify all 4 program files exist:
    Test-Path "$REPO/plc/Micro820_v3_Program.st"
    Test-Path "$REPO/plc/MbSrvConf_v3.xml"
    Test-Path "$REPO/plc/CCW_VARIABLES_v3.txt"
    Test-Path "$REPO/plc/CCW_DEPLOY_v3.txt"

4b. Show first 15 lines of the ST program to confirm it is v3.1:
    Get-Content "$REPO/plc/Micro820_v3_Program.st" | Select-Object -First 15

4c. Verify CCW project folder structure:
    Get-ChildItem "$CCW_PROJECT/Cosmos_Demo_v1.0" -Recurse -Name
...
6a. Tell me exactly:
    "Right-click Prog2 in the project tree -> Open.
     ...
     Open $REPO/plc/Micro820_v3_Program.st
     in Notepad, copy all, paste into CCW Prog2 editor."

6b. Tell me to load MbSrvConf_v3.xml:
    Copy from: $REPO/plc/MbSrvConf_v3.xml
    To: $CCW_PROJECT/Cosmos_Demo_v1.0/Controller/Controller/MbSrvConf.xml
...
7a. Tell me: Go Online (Ctrl+W) -> Connect to <PLC_LAN_IP>
...
7e. Verify via TCP:
    Test-NetConnection -ComputerName <PLC_LAN_IP> -Port 502
