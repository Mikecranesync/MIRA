...
def cmd_step2() -> None:
    ...
    result = subprocess.run(
        ["docker", "compose", "--profile", "test", "build", "telegram-test-runner"],
        cwd=BOTS_ROOT
    )
...
def cmd_step3() -> None:
    ...
    result = subprocess.run(
        ["docker", "run", "--rm",
         "-v", "telegram_test_session:/session",
         "alpine", "ls", "/session"],
    ...
    rc = _run_docker([
        "--manifest", "test_manifest_100.yaml", "--cases",
        "vfd_overcurrent_01", "plc_io_failure_21", "motor_overload_trip_41",
        "--timeout", "90",
    ])
    if rc == 0:
        _box([
...
def cmd_run10() -> None:
    ...
    if passed >= 8:
        ...
        if isinstance(results, list):
            for r in results:
                if not r.get("passed"):
                    print(f"  ❌  {r.get('case')}  bucket={r.get('failure_bucket')}  "
...
def cmd_run120() -> None:
    ...
    rc = _run_docker([
        "-v", f"{MANIFEST_V2}:/app/manifest_v2.yaml:ro",
        "--all", "--manifest", "manifest_v2.yaml",
    ])
...
def cmd_release() -> None:
    ...
    if tg_passed < 114:
        print(f"ERROR: Telegram pass rate {tg_passed}/120 below threshold (114). Run --run120 first.")
        sys.exit(1)
    if not ingest_ok:
...
def main() -> None:
    ...
    group.add_argument("--step2",   action="store_true", help="Build image + session setup instructions")
    ...
    group.add_argument("--release", action="store_true", help="Release gate — tags v2.0 if thresholds met")
