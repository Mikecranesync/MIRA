...
- ❌ **Reintroduce Anthropic into the diagnostic cascade** — removed PR #610, never reintroduce there. Cascade is Groq → Cerebras → Together. (Sole owner-authorized carve-out: PrintSynth print-vision interpretation, PR #2661 — vision on print photos only, never a chat/diagnosis provider.) **Amendment 2026-08-26 (Mike):** the free-tier cascade remains the fallback/dev default; **OpenAI API models are permitted behind the canonical server-side seam** (no direct client calls; central metering/policy/spend caps/audit/flags/kill switch); **Anthropic remains excluded** unless separately approved.
