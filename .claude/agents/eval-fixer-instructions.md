...
**De-dup rule (anti-inflation): there is exactly ONE rolling eval tracker issue.**
Do NOT open a new dated issue each run — that piled up 22+ stale duplicates by 2026-06.
Find the open tracker (stable title + `eval-tracker` label) and **comment** the latest
report on it. Only **create** it if no open tracker exists. Same pattern as
`.github/workflows/qa-regression.yml` (one running log).

```bash
STAMP=$(date +%Y-%m-%d)
TRACKER_TITLE="eval: offline scorecard — rolling tracker"

# The report body (one run). Comment this onto the tracker, or seed a new tracker with it.
REPORT_FILE="$(mktemp)"
cat > "$REPORT_FILE" <<EOF
## Eval failure report — ${STAMP}
...
# Find the single open tracker by stable title.
EXISTING=$(gh issue list --label eval-tracker --state open \
    --search "in:title \"${TRACKER_TITLE}\"" \
    --json number --jq '.[0].number // empty')

if [ -n "$EXISTING" ]; then
  # Append today's run as a comment — keeps one issue, full history.
  gh issue comment "$EXISTING" --body-file "$REPORT_FILE"
  ISSUE_URL=$(gh issue view "$EXISTING" --json url -q .url 2>/dev/null || echo "")
else
  # Seed the rolling tracker once. Ensure the label exists first.
  gh label create eval-tracker --color FBCA04 \
    --description "Rolling eval-fixer scorecard (one issue, commented each run)" 2>/dev/null || true
  gh issue create \
    --label eval-tracker \
    --title "${TRACKER_TITLE}" \
