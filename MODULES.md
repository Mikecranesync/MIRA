...
| packages | BENCH | no compose, no CI workflow; vitest/bun test suites run locally (`bun test` in each `packages/factorylm-*` dir) | Canonical shared FactoryLM UI shell foundation (theme, interaction, ui) — FACTORYLM-UNIFIED-UI-CUTOVER-001, `implemented_unconnected` per CAPABILITY_CLOSURE.yaml `unified_ui_shell`; see `.claude/rules/factorylm-unified-ui-cutover.md` |
| apps | BENCH | no compose, no CI workflow; contains only `factorylm-ui-lab` (disconnected fixture lab), local `bun test`/build only | Cross-surface verification harness for the unified UI shell — FACTORYLM-UNIFIED-UI-CUTOVER-001, `implemented_unconnected` per CAPABILITY_CLOSURE.yaml `unified_ui_shell`; see `.claude/rules/factorylm-unified-ui-cutover.md` |
